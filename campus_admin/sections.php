...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    // ✅ Get campus_id from logged-in campus admin
    $campus_id = $_SESSION['user']['linked_id'] ?? null;
    if (!$campus_id) {
      throw new Exception("Campus ID missing! Please re-login.");
    }
    ...
    if ($_POST['action'] === 'add') {
      ...
      if (!$faculty || !$dept || !$program || !$name || !$capacity)
        throw new Exception("All fields are required!");

      // ✅ Verify hierarchy belongs to same campus
      $verify = $pdo->prepare("
        ...
        WHERE f.faculty_id=? AND d.department_id=? AND p.program_id=? AND f.campus_id=?");
      $verify->execute([$faculty, $dept, $program, $campus_id]);
      if ($verify->fetchColumn() == 0)
        throw new Exception("Invalid selection: Faculty/Department/Program mismatch or belongs to another campus!");

      // ✅ Insert section with campus_id
      $stmt = $pdo->prepare("
        INSERT INTO class_section (faculty_id, department_id, program_id, campus_id, section_name, capacity, status)
        VALUES (?,?,?,?,?,?,?)");
      $stmt->execute([$faculty, $dept, $program, $campus_id, $name, $capacity, $status]);
    ...
    if ($_POST['action'] === 'update') {
      ...
      if (!$id || !$faculty || !$dept || !$program || !$name || !$capacity)
        throw new Exception("All fields are required!");
      ...
      $stmt = $pdo->prepare("
        ...
        SET faculty_id=?, department_id=?, program_id=?, campus_id=?, section_name=?, capacity=?, status=?, updated_at=NOW()
        WHERE section_id=?");
      $stmt->execute([$faculty, $dept, $program, $campus_id, $name, $capacity, $status, $id]);
    ...
    if ($_POST['action'] === 'delete') {
      ...
      $pdo->prepare("DELETE FROM class_section WHERE section_id=? AND campus_id=?")->execute([$id, $campus_id]);
...
if ($role === 'super_admin') {
  ...
  $programs = $pdo->query("SELECT * FROM programs ORDER BY program_name ASC")->fetchAll(PDO::FETCH_ASSOC);
  $sections = $pdo->query("
    SELECT s.*, f.faculty_name, d.department_name, p.program_name
  ...
  $faculties = $pdo->prepare("SELECT * FROM faculties WHERE campus_id=? ORDER BY faculty_name ASC");
...
<div class="main-content">
  ...
  <div class="table-wrapper">
    <table>
      <thead>
        <tr>
          <th>#</th><th>Faculty</th><th>Department</th><th>Program</th><th>Section</th><th>Capacity</th><th>Status</th><th>Actions</th>
        </tr>
      </thead>
      <tbody>
        ...
        <?php endforeach; else: ?>
        <tr><td colspan="8" style="text-align:center;color:#777;">No sections found.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
