...
/* ================= FETCH FILTER DATA WITH CAMPUS ADMIN RESTRICTION ================= */
$user_campus_id = null;
if ($_SESSION['user']['role'] === 'campus_admin') {
    // Get the campus_id from the user's linked_id
    $stmt = $pdo->prepare("SELECT linked_id FROM users WHERE user_id = ?");
    $stmt->execute([$_SESSION['user']['user_id']]);
    $user_campus_id = $stmt->fetchColumn();
    
    // Force the filter to only show their campus
    if (empty($_POST['campus_id'])) {
        $_POST['campus_id'] = $user_campus_id;
    }
    
    // Only show their campus
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['filter'])) {
  // If campus admin, force their campus ID
    ...
    <div class="grid">
      <div>
        ...
        <select name="campus_id" id="campus" onchange="loadFaculties()" 
            <?= ($_SESSION['user']['role'] === 'campus_admin') ? 'disabled' : 'required' ?>>
