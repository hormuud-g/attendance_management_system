...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] == 'get_faculties_by_campus') {
        ...
        // For campus_admin, restrict to their campus only
...
/* ================= FETCH INITIAL DATA ================= */
// For campus_admin, only show their campus
if ($_SESSION['user']['role'] === 'campus_admin' && $user_campus_id) {
    $campuses = $pdo->prepare("SELECT * FROM campus WHERE campus_id = ? AND status='active'");
...
// For campus_admin, restrict to promotions involving their campus
if ($_SESSION['user']['role'] === 'campus_admin' && $user_campus_id) {
    $where .= " AND (ph.old_campus_id = ? OR ph.new_campus_id = ?)";
    $params[] = $user_campus_id;
    $params[] = $user_campus_id;
    
    // Set campus filters to user's campus
    $_GET['old_campus_id'] = $_GET['old_campus_id'] ?? $user_campus_id;
    $_GET['new_campus_id'] = $_GET['new_campus_id'] ?? $user_campus_id;
}
        ...
        <div class="grid">
          <div>
            ...
            <select name="new_campus_id" id="new_campus" onchange="loadNewFaculties()" 
                <?= ($_SESSION['user']['role'] === 'campus_admin') ? 'disabled' : '' ?>>
              <option value="">All Campuses</option>
              <?php foreach($campuses as $c): ?>
                <option value="<?= $c['campus_id'] ?>" 
                    <?= (!empty($_GET['new_campus_id']) && $_GET['new_campus_id']==$c['campus_id'])?'selected':'' ?>
                    <?= ($_SESSION['user']['role'] === 'campus_admin' && isset($user_campus_id) && $user_campus_id == $c['campus_id'])?'selected':'' ?>>
                  <?= htmlspecialchars($c['campus_name']) ?>
                </option>
              <?php endforeach; ?>
            </select>
            <?php if ($_SESSION['user']['role'] === 'campus_admin' && isset($user_campus_id)): ?>
                <input type="hidden" name="new_campus_id" value="<?= $user_campus_id ?>">
            <?php endif; ?>
          </div>
  ...
  <div class="table-wrapper" id="reportArea">
    <div class="print-header">
      ...
      <div>
        ...
        <?php if ($_SESSION['user']['role'] === 'campus_admin' && isset($user_campus_id)): ?>
            <?php 
            $campus_name = $campuses[0]['campus_name'] ?? 'Campus';
            ?>
...
window.onload = function() {
    ...
    // Auto-load faculties if campus is selected
    const newCampus = document.getElementById('new_campus');
    if (newCampus && newCampus.value) {
        loadNewFaculties();
    }
};
</script>
