...
// ✅ Access control - Super Admin, Campus Admin, and Department Admin only
$allowed_roles = ['super_admin', 'campus_admin', 'department_admin'];
...
if ($role === 'campus_admin' && $user_campus_id) {
    // Campus admin - force filter to their campus
    $campus_filter = $user_campus_id;
} elseif ($role === 'department_admin' && $user_department_id) {
    // Department admin - force filter to their department
    $department_filter = $user_department_id;
    if ($user_campus_id) {
        $campus_filter = $user_campus_id;
    }
    if ($user_faculty_id) {
        $faculty_filter = $user_faculty_id;
    }
}
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] == 'get_departments') {
        ...
        if ($role === 'department_admin' && $user_department_id) {
            // Return only their department
            $stmt = $pdo->prepare("
                SELECT department_id, department_name 
                FROM departments 
                WHERE department_id = ? AND faculty_id = ? AND campus_id = ?
            ");
    ...
    if ($_GET['ajax'] == 'get_faculties') {
        ...
        if ($role === 'department_admin' && $user_campus_id && $campus_id != $user_campus_id) {
            echo json_encode(['status' => 'error', 'message' => 'Access denied']);
            exit;
        }
        
        if ($role === 'department_admin' && $user_faculty_id) {
            // Return only their faculty
            $stmt = $pdo->prepare("
                SELECT f.faculty_id, f.faculty_name 
                FROM faculties f
                JOIN faculty_campus fc ON f.faculty_id = fc.faculty_id
                WHERE fc.campus_id = ? AND f.faculty_id = ?
            ");
            $stmt->execute([$campus_id, $user_faculty_id]);
        } else {
            $stmt = $pdo->prepare("
    ...
    if ($_GET['ajax'] == 'get_programs') {
        ...
        if ($role === 'department_admin' && $user_department_id && $department_id != $user_department_id) {
            echo json_encode(['status' => 'error', 'message' => 'Access denied - You can only view programs in your department']);
            exit;
        }
    ...
    if ($_GET['ajax'] == 'get_classes') {
        ...
        if ($role === 'department_admin' && $user_department_id && $department_id != $user_department_id) {
            echo json_encode(['status' => 'error', 'message' => 'Access denied - You can only view classes in your department']);
            exit;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['action'] === 'add') {
        try {
            ...
            if ($role === 'department_admin' && $user_department_id) {
                ...
                if (empty($_POST['department_id']) || $_POST['department_id'] != $user_department_id) {
                    throw new Exception("You can only add subjects to your assigned department!");
                }
                ...
                if (empty($_POST['faculty_id']) || $_POST['faculty_id'] != $user_faculty_id) {
    ...
    if ($_POST['action'] === 'update') {
        try {
            ...
            if ($role === 'department_admin' && $user_department_id) {
                // Check if the subject belongs to user's department
                $check_dept = $pdo->prepare("
                    SELECT department_id, campus_id, faculty_id 
                    FROM subject 
                    WHERE subject_id = ?
                ");
                $check_dept->execute([$_POST['subject_id']]);
                $subject_data = $check_dept->fetch(PDO::FETCH_ASSOC);
                
                if ($subject_data['department_id'] != $user_department_id) {
                    throw new Exception("You can only update subjects in your assigned department!");
                }
                
                // Ensure they're not trying to change department, campus, or faculty
                if ($_POST['department_id'] != $user_department_id) {
                    throw new Exception("You cannot move subjects to another department!");
                }
                ...
                if ($_POST['faculty_id'] != $user_faculty_id) {
                    throw new Exception("You cannot move subjects to another faculty!");
                }
            }
    ...
    if ($_POST['action'] === 'delete') {
        try {
            ...
            if ($role === 'department_admin' && $user_department_id) {
                // Check if the subject belongs to user's department
                $check_dept = $pdo->prepare("
                    SELECT department_id FROM subject WHERE subject_id = ?
                ");
                $check_dept->execute([$_POST['subject_id']]);
                $subject_dept = $check_dept->fetchColumn();
                
                if ($subject_dept != $user_department_id) {
                    throw new Exception("You can only delete subjects from your assigned department!");
                }
            }
...
} elseif ($role === 'department_admin' && $user_campus_id) {
    $campuses = $pdo->prepare("SELECT * FROM campus WHERE campus_id = ? ORDER BY campus_name ASC");
    $campuses->execute([$user_campus_id]);
    $campuses = $campuses->fetchAll(PDO::FETCH_ASSOC);
} else {
...
} elseif ($role === 'department_admin' && $user_faculty_id) {
    $faculties = $pdo->prepare("
        SELECT faculty_id, faculty_name 
        FROM faculties 
        WHERE faculty_id = ?
    ");
    $faculties->execute([$user_faculty_id]);
    $faculties = $faculties->fetchAll(PDO::FETCH_ASSOC);
} else {
...
if ($role === 'campus_admin' && $user_campus_id) {
    ...
    $departments = $pdo->prepare("
        ...
        WHERE department_id = ?
    ");
    $departments->execute([$user_department_id]);
    $departments = $departments->fetchAll(PDO::FETCH_ASSOC);
} else {
...
// Get all classes for filter - with role-based restrictions
if ($role === 'campus_admin' && $user_campus_id) {
    ...
    $all_classes = $pdo->prepare("
        ...
        FROM classes c
        JOIN programs p ON c.program_id = p.program_id
        WHERE c.status = 'Active' AND c.department_id = ?
        ORDER BY c.class_name
    ");
    $all_classes->execute([$user_department_id]);
    $all_classes = $all_classes->fetchAll(PDO::FETCH_ASSOC);
} else {
...
// Get all programs for filter - with role-based restrictions
if ($role === 'campus_admin' && $user_campus_id) {
    ...
    $all_programs = $pdo->prepare("
        ...
        ORDER BY program_name ASC
    ");
    $all_programs->execute([$user_department_id]);
    $all_programs = $all_programs->fetchAll(PDO::FETCH_ASSOC);
} else {
...
// Faculty filter (only for super_admin and campus_admin)
if (!empty($faculty_filter) && ($role === 'super_admin' || $role === 'campus_admin')) {
    $query .= " AND s.faculty_id = ?";
    $params[] = $faculty_filter;
}

// Department filter (only for super_admin and campus_admin)
if (!empty($department_filter) && ($role === 'super_admin' || $role === 'campus_admin')) {
    $query .= " AND s.department_id = ?";
    $params[] = $department_filter;
}

// Program filter (only for super_admin, campus_admin, and department_admin)
if (!empty($program_filter) && ($role === 'super_admin' || $role === 'campus_admin' || $role === 'department_admin')) {
    $query .= " AND s.program_id = ?";
    $params[] = $program_filter;
}

// Class filter (only for super_admin, campus_admin, and department_admin)
if (!empty($class_filter) && ($role === 'super_admin' || $role === 'campus_admin' || $role === 'department_admin')) {
...
// Get stats for department admin
if ($role === 'department_admin' && $user_department_id) {
    $total_subjects = $pdo->prepare("SELECT COUNT(*) FROM subject WHERE department_id = ?");
    $total_subjects->execute([$user_department_id]);
    $total_subjects = $total_subjects->fetchColumn();
    
    $active_subjects = $pdo->prepare("SELECT COUNT(*) FROM subject WHERE status='active' AND department_id = ?");
    $active_subjects->execute([$user_department_id]);
    $active_subjects = $active_subjects->fetchColumn();
    
    $inactive_subjects = $pdo->prepare("SELECT COUNT(*) FROM subject WHERE status='inactive' AND department_id = ?");
    $inactive_subjects->execute([$user_department_id]);
    $inactive_subjects = $inactive_subjects->fetchColumn();
}
...
/* Stats cards for department admin */
.stats-container {
  display: grid;
...
<div class="main-content">
  <div class="page-header">
    ...
    <?php if ($role === 'super_admin' || ($role === 'campus_admin' && $user_campus_id) || ($role === 'department_admin' && $user_department_id)): ?>
  ...
  <!-- Stats cards for department admin (optional - you can uncomment if needed) -->
  <?php if ($role === 'department_admin' && isset($total_subjects)): ?>
  <!-- <div class="stats-container">
    ...
    <div class="stat-card inactive">
      ...
      <div class="stat-info">
        <h3>Inactive Subjects</h3>
        <div class="number"><?= $inactive_subjects ?></div>
      </div>
    </div>
  </div> -->
  <?php endif; ?>
  ...
  <div class="filters-container">
    ...
    <form method="GET" class="filter-form" id="filterForm">
        ...
        <!-- Campus Filter (disabled for campus_admin and department_admin) -->
        <div class="filter-group">
            ...
            <div style="position:relative;">
                ...
                <?php if ($role !== 'super_admin' && $user_campus_id): ?>
                <input type="hidden" name="campus" value="<?= $user_campus_id ?>">
                <?php endif; ?>
            </div>
        </div>
        
        <!-- Faculty Filter (disabled for department_admin) -->
        <div class="filter-group">
            <label for="faculty">Faculty</label>
            <div style="position:relative;">
                <select id="faculty" name="faculty" class="filter-input" 
                        <?= ($role === 'department_admin') ? 'disabled' : '' ?>>
                ...
                <?php if ($role === 'department_admin' && $user_faculty_id): ?>
                <input type="hidden" name="faculty" value="<?= $user_faculty_id ?>">
                <?php endif; ?>
            </div>
        </div>
        
        <!-- Department Filter (disabled for department_admin) -->
        <div class="filter-group">
            ...
            <div style="position:relative;">
                ...
                <?php if ($role === 'department_admin' && $user_department_id): ?>
            ...
            <tr>
              ...
              <td>
                <div class="action-btns">
                  ...
                  } elseif ($role === 'campus_admin' && $user_campus_id && $s['campus_id'] == $user_campus_id) {
                      $can_edit_delete = true;
                  } elseif ($role === 'department_admin' && $user_department_id && $s['department_id'] == $user_department_id) {
                      $can_edit_delete = true;
                  }
      ...
      <?php if($subjects): ?>
        ...
        <div class="subject-card">
          ...
          <div class="subject-actions">
            <?php 
            $can_edit_delete_mobile = false;
            
            if ($role === 'super_admin') {
                $can_edit_delete_mobile = true;
            } elseif ($role === 'campus_admin' && $user_campus_id && $s['campus_id'] == $user_campus_id) {
                $can_edit_delete_mobile = true;
            } elseif ($role === 'department_admin' && $user_department_id && $s['department_id'] == $user_department_id) {
                $can_edit_delete_mobile = true;
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select id="campus_id" name="campus_id" class="form-control" required 
            ...
            <?php foreach($campuses as $campus): ?>
            <option value="<?= $campus['campus_id'] ?>" 
                <?= ($role !== 'super_admin' && $user_campus_id == $campus['campus_id']) ? 'selected' : '' ?>>
              <?= htmlspecialchars($campus['campus_name']) ?>
            </option>
            <?php endforeach; ?>
          </select>
        ...
        <div class="form-group">
          ...
          <select id="faculty_id" name="faculty_id" class="form-control" required 
                  onchange="onFacultyChange('add')" 
                  <?= ($role === 'department_admin') ? 'disabled' : '' ?>>
            <option value="">Select Faculty</option>
          </select>
          <?php if ($role === 'department_admin' && $user_faculty_id): ?>
          <input type="hidden" id="faculty_id_hidden" name="faculty_id" value="<?= $user_faculty_id ?>">
          <?php endif; ?>
        </div>
        ...
        <div class="form-group">
          ...
          <select id="department_id" name="department_id" class="form-control" required 
                  onchange="onDepartmentChange('add')" 
                  <?= ($role === 'department_admin') ? 'disabled' : '' ?>>
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <select id="edit_faculty_id" name="faculty_id" class="form-control" required 
                  onchange="onFacultyChange('edit')" 
                  <?= ($role === 'department_admin') ? 'disabled' : '' ?>>
            <option value="">Select Faculty</option>
          </select>
          <?php if ($role === 'department_admin' && $user_faculty_id): ?>
          <input type="hidden" name="faculty_id" value="<?= $user_faculty_id ?>">
          <?php endif; ?>
        </div>
        ...
        <div class="form-group">
          ...
          <select id="edit_department_id" name="department_id" class="form-control" required 
                  onchange="onDepartmentChange('edit')" 
                  <?= ($role === 'department_admin') ? 'disabled' : '' ?>>
            <option value="">Select Department</option>
          </select>
          <?php if ($role === 'department_admin' && $user_department_id): ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php if ($role === 'department_admin' && $user_campus_id && $user_faculty_id && $user_department_id): ?>
    // Auto-load data for department admin
    setTimeout(function() {
        ...
        if (campusSelect) {
            // Trigger faculty load
            loadFacultiesByCampus('add', <?= $user_campus_id ?>, <?= $user_faculty_id ?>, <?= $user_department_id ?>);
        }
    }, 500);
    <?php endif; ?>
});
