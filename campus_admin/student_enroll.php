...
/* =============== FETCH INITIAL DATA =============== */
// ✅ CAMPUS SELECTION (super_admin only)
if ($role === 'super_admin') {
    $campuses = $pdo->query("
...
$selectedCampus = $_POST['campus_id'] ?? $_GET['campus_id'] ?? ($role === 'campus_admin' ? $campus_id : null);
$selectedFaculty = $_POST['faculty_id'] ?? $_GET['faculty_id'] ?? null;
...
<div class="main-content">
    <div class="page-header">
        <h1>
            ...
            <span style="font-size: 14px; background: #f0f0f0; padding: 5px 10px; border-radius: 20px; color: #666;">
                Super Admin
            </span>
            <?php endif; ?>
        </h1>
            ...
            <div class="grid">
                ...
                <div>
                    ...
                    <select name="faculty_id" id="faculty_id" required 
                        ...
                        <?php 
                        // Load faculties if campus is already selected
                ...
                <div>
                    ...
                    <select name="department_id" id="department_id" required 
                        ...
                        <?php 
                        if (!empty($selectedFaculty) && !empty($selectedCampus)) {
    ...
    <?php if (!empty($selectedProgram) && !empty($selectedSemester) && !empty($selectedClass)): 
        ...
        // ✅ FIXED: Build base query with filters - Get students in this specific class using class_id
        $whereClauses = [
            "s.status = 'active'", // Only active students
            "s.program_id = ?",
            "s.campus_id = ?",
            "s.faculty_id = ?",
            "s.department_id = ?",
            "s.class_id = ?" // IMPORTANT: This ensures we only get students in the selected class
        ];
        ...
        $params = [
            ...
            $selectedFaculty, 
            $selectedDepartment,
            $class_id  // This is the selected class ID
        ];
        
        // ✅ FIXED: Check enrollment status - students NOT enrolled in these subjects for this term
