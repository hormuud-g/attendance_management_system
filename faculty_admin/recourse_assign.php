...
// ✅ Access Control - Only super_admin and faculty_admin can access
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] == 'get_faculties_by_campus') {
        ...
        // If faculty_admin, only show their faculty
        if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
            ...
            ");
            $stmt->execute([$campus_id, $faculty_id]);
        } else {
            $stmt = $pdo->prepare("
    ...
    if ($_GET['ajax'] == 'get_departments_by_faculty') {
        ...
        // If faculty_admin, only show departments from their faculty
        if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
            if ($faculty_id_param != $faculty_id) {
        ...
        ");
        $stmt->execute([$faculty_id_param, $campus_id]);
    ...
    if ($_GET['ajax'] == 'get_programs_by_department') {
        ...
        // If faculty_admin, only show programs from their faculty
        if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
            if ($faculty_id_param != $faculty_id) {
        ...
        ");
        $stmt->execute([$department_id, $faculty_id_param, $campus_id]);
    ...
    if ($_GET['ajax'] == 'get_study_groups_by_program') {
        ...
        // If faculty_admin, only show classes from their faculty
        if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
            if ($faculty_id_param != $faculty_id) {
        ...
        $stmt = $pdo->prepare("
            ...
            ORDER BY class_name
        ");
        $stmt->execute([$program_id, $department_id, $faculty_id_param, $campus_id]);
    ...
    if ($_GET['ajax'] == 'get_students_by_filters') {
        ...
        // If faculty_admin, only show students from their faculty
        if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
            $where[] = "s.faculty_id = ?";
            $params[] = $faculty_id;
        }
        ...
        if (!empty($faculty_id_param) && $current_user_role !== 'faculty_admin') {
            $where[] = "s.faculty_id = ?";
            $params[] = $faculty_id_param;
        }
    ...
    if ($_GET['ajax'] == 'get_subjects_by_full_hierarchy') {
        ...
        // If faculty_admin, check faculty access
        if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
            if ($faculty_id_param != $faculty_id) {
        ...
        if (!$campus_id || !$faculty_id_param || !$department_id || !$program_id || !$class_id || !$semester_id) {
            echo json_encode(['status' => 'success', 'subjects' => []]);
            exit;
        }
        ...
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$campus_id, $faculty_id_param, $department_id, $program_id, $class_id, $semester_id]);
    ...
    if ($_GET['ajax'] == 'get_statistics') {
        ...
        // If faculty_admin, only count their faculty's records
        if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM recourse_student rs
...
// If faculty_admin, only show their faculty's data
if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        if ($_POST['action'] === 'delete') {
            // Check if faculty_admin has permission to delete this record
            if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
                $check_stmt = $pdo->prepare("
                    SELECT COUNT(*) FROM recourse_student rs
                    JOIN students s ON rs.student_id = s.student_id
                    WHERE rs.recourse_id = ? AND s.faculty_id = ?
                ");
                $check_stmt->execute([$_POST['recourse_id'], $faculty_id]);
                if ($check_stmt->fetchColumn() == 0) {
                    throw new Exception("❌ You don't have permission to delete this record!");
                }
            }
            ...
            // Check if faculty_admin has permission to add for this student
            if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
                if ($student['faculty_id'] != $faculty_id) {
                    throw new Exception("❌ You can only add recourses for students in your faculty!");
                }
            }
            ...
            // Check if faculty_admin has permission to update this record
            if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
                $check_stmt = $pdo->prepare("
                    SELECT COUNT(*) FROM recourse_student rs
                    JOIN students s ON rs.student_id = s.student_id
                ...
                $check_stmt->execute([$_POST['recourse_id'], $faculty_id]);
                if ($check_stmt->fetchColumn() == 0) {
                    throw new Exception("❌ You don't have permission to update this record!");
                }
            }
...
// For faculty_admin, only show relevant campuses
if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
...
// For faculty_admin, only show subjects from their faculty
if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
    $subjects = $pdo->prepare("SELECT * FROM subject WHERE status='active' AND faculty_id = ? ORDER BY subject_code ASC");
...
if (!empty($filter_campus)) {
    if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
        $stmt = $pdo->prepare("
            ...
            FROM faculties f
            JOIN faculty_campus fc ON f.faculty_id = fc.faculty_id
            WHERE fc.campus_id = ? AND f.faculty_id = ? AND f.status = 'active'
            ORDER BY f.faculty_name
        ");
        $stmt->execute([$filter_campus, $faculty_id]);
    } else {
        $stmt = $pdo->prepare("
            SELECT DISTINCT f.faculty_id, f.faculty_name 
            FROM faculties f
...
if (!empty($filter_faculty) && !empty($filter_campus)) {
    // Check if faculty_admin has permission for this faculty
    if ($current_user_role === 'faculty_admin' && $faculty_id > 0 && $filter_faculty != $faculty_id) {
        $filter_departments = [];
    } else {
        $stmt = $pdo->prepare("
            SELECT department_id, department_name 
...
if ($current_user_role === 'faculty_admin' && $faculty_id > 0) {
    $total_recourses = $pdo->prepare("SELECT COUNT(*) FROM recourse_student rs
                                     JOIN students s ON rs.student_id = s.student_id
                                     WHERE s.faculty_id = ?");
    $total_recourses->execute([$faculty_id]);
    $total_recourses = $total_recourses->fetchColumn();
    
    $active_recourses = $pdo->prepare("SELECT COUNT(*) FROM recourse_student rs
                                      JOIN students s ON rs.student_id = s.student_id
                                      WHERE s.faculty_id = ? AND rs.status = 'active'");
    $active_recourses->execute([$faculty_id]);
    $active_recourses = $active_recourses->fetchColumn();
    
    $completed_recourses = $pdo->prepare("SELECT COUNT(*) FROM recourse_student rs
                                        JOIN students s ON rs.student_id = s.student_id
                                        WHERE s.faculty_id = ? AND rs.status = 'completed'");
    $completed_recourses->execute([$faculty_id]);
    $completed_recourses = $completed_recourses->fetchColumn();
    
    $cancelled_recourses = $pdo->prepare("SELECT COUNT(*) FROM recourse_student rs
                                        JOIN students s ON rs.student_id = s.student_id
                                        WHERE s.faculty_id = ? AND rs.status = 'cancelled'");
    $cancelled_recourses->execute([$faculty_id]);
    $cancelled_recourses = $cancelled_recourses->fetchColumn();
} else {
...
<head>
    ...
    <title>Recourse Students Management - <?= $faculty_name ? htmlspecialchars($faculty_name) : 'All Faculties' ?></title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
    /* ===========================================
       MAIN LAYOUT & STRUCTURE - Same as before
    =========================================== */
    * {
        margin: 0;
        padding: 0;
    ...
    /* ===========================================
       BUTTONS - ENHANCED (Same as before)
    =========================================== */
    .btn {
        border: none;
        padding: 10px 18px;
    ...
    /* ===========================================
       ALERTS - ENHANCED (Same as before)
    =========================================== */
    .alert {
        position: fixed;
        top: 50%;
    ...
    /* ===========================================
       FILTER BOX - ENHANCED (Same as before)
    =========================================== */
    .filter-box {
        background: white;
        padding: 25px;
    ...
    /* ===========================================
       TABLE STYLES - ENHANCED (Same as before)
    =========================================== */
    .table-responsive {
        width: 100%;
        overflow-x: auto;
    ...
    /* ===========================================
       STATUS BADGES - ENHANCED (Same as before)
    =========================================== */
    .status-badge {
    ...
    /* ===========================================
       ACTION BUTTONS - ENHANCED (Same as before)
    =========================================== */
    .action-buttons {
        display: flex;
        gap: 8px;
    ...
    /* ===========================================
       MODAL STYLES - ENHANCED (Same as before)
    =========================================== */
    .modal {
        display: none;
        position: fixed;
    ...
    /* ===========================================
       HIERARCHY SECTION STYLING (Same as before)
    =========================================== */
    .hierarchy-section {
        grid-column: span 4;
        background: linear-gradient(135deg, #f8f9fa, #e9ecef);
    ...
    /* ===========================================
       CUSTOM SCROLLBAR FOR MODAL (Same as before)
    =========================================== */
    ...
    /* ===========================================
       FORM VALIDATION STYLES (Same as before)
    =========================================== */
    input:invalid,
    select:invalid,
    textarea:invalid {
    ...
    /* ===========================================
       REASON TEXT TRUNCATION (Same as before)
    =========================================== */
    .reason-text {
        cursor: help;
    ...
    /* ===========================================
       BREADCRUMB STYLE (Same as before)
    =========================================== */
    .breadcrumb {
        display: flex;
        align-items: center;
...
<body>
    ...
    let currentFacultyId = <?= $faculty_id ?: 'null' ?>;
    ...
    async function editRecourse(recourseData) {
        // For faculty_admin, check if they have permission to edit this record
        if (currentUserRole === 'faculty_admin' && currentFacultyId) {
            if (recourseData.original_faculty_id != currentFacultyId) {
                alert('❌ You can only edit recourse records for students in your faculty!');
                return;
            }
        }
