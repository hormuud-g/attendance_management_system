...
// Check user role - Extended to include department_admin
...
$user_linked_table = $_SESSION['user']['linked_table'] ?? null;
$user_campus_id = null;
$user_faculty_id = null;
$user_department_id = null;

// Get user's hierarchy based on role
if ($user_role === 'campus_admin' && $user_linked_table === 'campus') {
    $user_campus_id = $user_linked_id;
} 
else if ($user_role === 'faculty_admin' && $user_linked_table === 'faculty') {
    $user_faculty_id = $user_linked_id;
...
else if ($user_role === 'department_admin' && $user_linked_table === 'department') {
    $user_department_id = $user_linked_id;
    // Get faculty_id and campus_id from department
    $stmt = $pdo->prepare("SELECT faculty_id, campus_id FROM departments WHERE department_id = ?");
    $stmt->execute([$user_department_id]);
    $dept = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($dept) {
        $user_faculty_id = $dept['faculty_id'];
        $user_campus_id = $dept['campus_id'];
    }
}
...
if (isset($_GET['ajax'])) {
    ...
    // Security check for different admin roles in AJAX requests
    if ($user_role === 'campus_admin' && $user_campus_id) {
        if (isset($_GET['campus_id']) && $_GET['campus_id'] != $user_campus_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized access to this campus']);
            exit;
        }
    }
    else if ($user_role === 'faculty_admin' && $user_faculty_id) {
        if (isset($_GET['faculty_id']) && $_GET['faculty_id'] != $user_faculty_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized access to this faculty']);
            exit;
        }
        if (isset($_GET['campus_id']) && $_GET['campus_id'] != $user_campus_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized access to this campus']);
    ...
    else if ($user_role === 'department_admin' && $user_department_id) {
        if (isset($_GET['department_id']) && $_GET['department_id'] != $user_department_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized access to this department']);
            exit;
        }
        if (isset($_GET['faculty_id']) && $_GET['faculty_id'] != $user_faculty_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized access to this faculty']);
    ...
    // GET CAMPUSES BY USER ROLE
    if ($_GET['ajax'] == 'get_user_campuses') {
        $campuses = [];
        
        if ($user_role === 'super_admin') {
            $stmt = $pdo->query("SELECT campus_id, campus_name FROM campus WHERE status = 'active' ORDER BY campus_name");
            $campuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } 
        else if ($user_role === 'campus_admin' && $user_campus_id) {
            $stmt = $pdo->prepare("SELECT campus_id, campus_name FROM campus WHERE campus_id = ? AND status = 'active'");
            $stmt->execute([$user_campus_id]);
            $campuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
        else if ($user_role === 'faculty_admin' && $user_campus_id) {
            $stmt = $pdo->prepare("SELECT campus_id, campus_name FROM campus WHERE campus_id = ? AND status = 'active'");
            $stmt->execute([$user_campus_id]);
            $campuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
        else if ($user_role === 'department_admin' && $user_campus_id) {
            $stmt = $pdo->prepare("SELECT campus_id, campus_name FROM campus WHERE campus_id = ? AND status = 'active'");
            $stmt->execute([$user_campus_id]);
            $campuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
    ...
    if ($_GET['ajax'] == 'get_faculties_by_campus') {
        ...
        // Additional filter for faculty_admin and department_admin
        if ($user_role === 'faculty_admin' && $user_faculty_id) {
            $sql .= " AND f.faculty_id = ?";
            $params = [$campus_id, $user_faculty_id];
        } 
        else if ($user_role === 'department_admin' && $user_faculty_id) {
            $sql .= " AND f.faculty_id = ?";
            $params = [$campus_id, $user_faculty_id];
        }
        else {
    ...
    if ($_GET['ajax'] == 'get_departments_by_faculty') {
        ...
        // Additional filter for department_admin
        if ($user_role === 'department_admin' && $user_department_id) {
            $sql .= " AND department_id = ?";
            $params = [$faculty_id, $campus_id, $user_department_id];
        } else {
    ...
    if ($_GET['ajax'] == 'get_all_classes_by_campus') {
        ...
        if ($user_role === 'faculty_admin' && $user_faculty_id) {
            ...
            $params = [$campus_id, $user_department_id];
        }
        else {
            $params = [$campus_id];
        }
    ...
    if ($_GET['ajax'] == 'get_students_by_filters') {
        ...
        $sql = "SELECT s.student_id, s.reg_no, s.full_name, 
                       s.campus_id, s.faculty_id, s.department_id, s.program_id, s.semester_id,
    ...
    // GET SUBJECTS BY FULL HIERARCHY
    ...
    if ($_GET['ajax'] == 'get_class_details') {
        ...
        // Security check for different roles
        if ($class) {
            if ($user_role === 'campus_admin' && $user_campus_id && $class['campus_id'] != $user_campus_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
            if ($user_role === 'faculty_admin' && $user_faculty_id && $class['faculty_id'] != $user_faculty_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
            if ($user_role === 'department_admin' && $user_department_id && $class['department_id'] != $user_department_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
        }
    ...
    if ($_GET['ajax'] == 'get_student_details') {
        ...
        // Security check for different roles
        if ($student) {
            if ($user_role === 'campus_admin' && $user_campus_id && $student['campus_id'] != $user_campus_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
            if ($user_role === 'faculty_admin' && $user_faculty_id && $student['faculty_id'] != $user_faculty_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
            if ($user_role === 'department_admin' && $user_department_id && $student['department_id'] != $user_department_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
        }
    ...
    if ($_GET['ajax'] == 'get_recourse_hierarchy') {
        ...
        // Security check for different roles
        if ($recourse) {
            if ($user_role === 'campus_admin' && $user_campus_id && $recourse['recourse_campus_id'] != $user_campus_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
            if ($user_role === 'faculty_admin' && $user_faculty_id && $recourse['recourse_faculty_id'] != $user_faculty_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
            if ($user_role === 'department_admin' && $user_department_id && $recourse['recourse_department_id'] != $user_department_id) {
                echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
                exit;
            }
        }
    ...
    if ($_GET['ajax'] == 'get_statistics') {
        ...
        if ($user_role === 'super_admin') {
            ...
            $where[] = "recourse_campus_id = ?";
            $params[] = $user_campus_id;
        }
        else if ($user_role === 'faculty_admin' && $user_faculty_id) {
            $where[] = "recourse_faculty_id = ?";
            $params[] = $user_faculty_id;
        }
        else if ($user_role === 'department_admin' && $user_department_id) {
            $where[] = "recourse_department_id = ?";
        ...
        // Total count
        $totalQuery = "SELECT COUNT(*) FROM recourse_student" . $whereClause;
        $totalStmt = $pdo->prepare($totalQuery);
        $totalStmt->execute($params);
        $total = $totalStmt->fetchColumn();
        
        // Active count
        $activeQuery = "SELECT COUNT(*) FROM recourse_student WHERE status = 'active'";
        if (!empty($where)) {
            $activeQuery .= " AND " . implode(" AND ", $where);
        }
        ...
        // Completed count
        $completedQuery = "SELECT COUNT(*) FROM recourse_student WHERE status = 'completed'";
        if (!empty($where)) {
            $completedQuery .= " AND " . implode(" AND ", $where);
        }
        ...
        // Cancelled count
        $cancelledQuery = "SELECT COUNT(*) FROM recourse_student WHERE status = 'cancelled'";
        if (!empty($where)) {
            $cancelledQuery .= " AND " . implode(" AND ", $where);
        }
...
if ($user_role === 'campus_admin' && $user_campus_id) {
    $filter_campus = $user_campus_id;
} 
else if ($user_role === 'faculty_admin' && $user_faculty_id) {
    $filter_faculty = $user_faculty_id;
    $filter_campus = $user_campus_id;
}
else if ($user_role === 'department_admin' && $user_department_id) {
    $filter_department = $user_department_id;
    $filter_faculty = $user_faculty_id;
    $filter_campus = $user_campus_id;
}
...
if ($user_role === 'campus_admin' && $user_campus_id) {
    $whereConditions[] = "rs.recourse_campus_id = ?";
    $params[] = $user_campus_id;
}
else if ($user_role === 'faculty_admin' && $user_faculty_id) {
    $whereConditions[] = "rs.recourse_faculty_id = ?";
    $params[] = $user_faculty_id;
}
else if ($user_role === 'department_admin' && $user_department_id) {
    $whereConditions[] = "rs.recourse_department_id = ?";
    $params[] = $user_department_id;
}
...
if (!empty($filter_program)) {
    $whereConditions[] = "rs.recourse_program_id = ?";
    $params[] = $filter_program;
}

if (!empty($filter_class)) {
    $whereConditions[] = "rs.recourse_class_id = ?";
    $params[] = $filter_class;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        // Security check for different admin roles
        if ($_POST['action'] === 'add' || $_POST['action'] === 'update') {
            $recourse_campus_id = $_POST['recourse_campus_id'];
            $recourse_faculty_id = $_POST['recourse_faculty_id'];
            $recourse_department_id = $_POST['recourse_department_id'];
            
            // Check permissions based on user role
            if ($user_role === 'campus_admin' && $user_campus_id) {
                if ($recourse_campus_id != $user_campus_id) {
                    throw new Exception("❌ You are only authorized to add recourse for your campus!");
                }
            }
            else if ($user_role === 'faculty_admin' && $user_faculty_id) {
                if ($recourse_faculty_id != $user_faculty_id) {
                    throw new Exception("❌ You are only authorized to add recourse for your faculty!");
                }
            }
            else if ($user_role === 'department_admin' && $user_department_id) {
                if ($recourse_department_id != $user_department_id) {
                    throw new Exception("❌ You are only authorized to add recourse for your department!");
                }
            }
            
            // Also check student's hierarchy
            $student_check = $pdo->prepare("
                SELECT campus_id, faculty_id, department_id 
                FROM students 
                WHERE student_id = ?
            ");
            $student_check->execute([$_POST['student_id']]);
            ...
            // Check if student belongs to user's hierarchy
            if ($user_role === 'campus_admin' && $user_campus_id && $student['campus_id'] != $user_campus_id) {
                throw new Exception("❌ This student is not from your campus!");
            }
            else if ($user_role === 'faculty_admin' && $user_faculty_id && $student['faculty_id'] != $user_faculty_id) {
                throw new Exception("❌ This student is not from your faculty!");
            }
            else if ($user_role === 'department_admin' && $user_department_id && $student['department_id'] != $user_department_id) {
                throw new Exception("❌ This student is not from your department!");
            }
        }
        else if ($_POST['action'] === 'delete') {
            // Check if the recourse record belongs to user's hierarchy
            $recourse_check = $pdo->prepare("
                SELECT recourse_campus_id, recourse_faculty_id, recourse_department_id 
                FROM recourse_student 
                WHERE recourse_id = ?
            ");
            $recourse_check->execute([$_POST['recourse_id']]);
            $recourse = $recourse_check->fetch(PDO::FETCH_ASSOC);
            
            if (!$recourse) {
                throw new Exception("❌ Recourse record not found!");
            }
            
            if ($user_role === 'campus_admin' && $user_campus_id && $recourse['recourse_campus_id'] != $user_campus_id) {
                throw new Exception("❌ You are not authorized to delete this record!");
            }
            else if ($user_role === 'faculty_admin' && $user_faculty_id && $recourse['recourse_faculty_id'] != $user_faculty_id) {
                throw new Exception("❌ You are not authorized to delete this record!");
            }
            else if ($user_role === 'department_admin' && $user_department_id && $recourse['recourse_department_id'] != $user_department_id) {
                throw new Exception("❌ You are not authorized to delete this record!");
            }
        }
        ...
        if ($_POST['action'] === 'delete') {
            ...
            // Get student's current details for original information
            $student_info = $pdo->prepare("
                SELECT campus_id, faculty_id, department_id, program_id, class_id, semester_id 
                FROM students 
                WHERE student_id = ?
            ");
...
// Get all active data for dropdowns
...
// Get user's hierarchy names for display
$user_campus_name = '';
$user_faculty_name = '';
...
if ($user_role === 'campus_admin' && $user_campus_id) {
    foreach($campuses as $c) {
        if ($c['campus_id'] == $user_campus_id) {
            $user_campus_name = $c['campus_name'];
            break;
        }
    }
}
    ...
    $stmt->execute([$user_faculty_id]);
    $user_faculty_name = $stmt->fetchColumn();
}
else if ($user_role === 'department_admin' && $user_department_id) {
    foreach($campuses as $c) {
        if ($c['campus_id'] == $user_campus_id) {
...
if (!empty($filter_campus)) {
    $sql = "
        ...
        AND f.status = 'active'
    ";
    $filter_params = [$filter_campus];
    
    if ($user_role === 'faculty_admin' && $user_faculty_id) {
        $sql .= " AND f.faculty_id = ?";
        $filter_params[] = $user_faculty_id;
    }
...
if (!empty($filter_faculty) && !empty($filter_campus)) {
    $sql = "
        ...
        AND status = 'active'
    ";
    $filter_params = [$filter_faculty, $filter_campus];
    
    if ($user_role === 'department_admin' && $user_department_id) {
        $sql .= " AND department_id = ?";
        $filter_params[] = $user_department_id;
    }
...
// Get counts for statistics - FIXED: Use separate parameter arrays for each query
$stats_params = [];
...
if ($user_role === 'super_admin') {
    ...
    $stats_total_params = [];
    $stats_active_params = [];
    $stats_completed_params = [];
    $stats_cancelled_params = [];
    
    if ($user_role === 'campus_admin' && $user_campus_id) {
        $stats_conditions[] = "recourse_campus_id = ?";
        $stats_total_params[] = $user_campus_id;
        $stats_active_params[] = $user_campus_id;
        $stats_completed_params[] = $user_campus_id;
        $stats_cancelled_params[] = $user_campus_id;
    }
    else if ($user_role === 'faculty_admin' && $user_faculty_id) {
        $stats_conditions[] = "recourse_faculty_id = ?";
        $stats_total_params[] = $user_faculty_id;
        $stats_active_params[] = $user_faculty_id;
        $stats_completed_params[] = $user_faculty_id;
        $stats_cancelled_params[] = $user_faculty_id;
    }
    else if ($user_role === 'department_admin' && $user_department_id) {
        $stats_conditions[] = "recourse_department_id = ?";
        $stats_total_params[] = $user_department_id;
        $stats_active_params[] = $user_department_id;
        $stats_completed_params[] = $user_department_id;
        $stats_cancelled_params[] = $user_department_id;
    }
    
    $stats_where_clause = !empty($stats_conditions) ? " WHERE " . implode(" AND ", $stats_conditions) : "";
    
    // Total count
    $total_query = "SELECT COUNT(*) FROM recourse_student" . $stats_where_clause;
    $total_stmt = $pdo->prepare($total_query);
    $total_stmt->execute($stats_total_params);
    $total_recourses = $total_stmt->fetchColumn();
    
    // Active count
    if (!empty($stats_conditions)) {
        $active_query = "SELECT COUNT(*) FROM recourse_student WHERE status = 'active' AND " . implode(" AND ", $stats_conditions);
        $active_stmt = $pdo->prepare($active_query);
        $active_stmt->execute($stats_active_params);
    } else {
        $active_query = "SELECT COUNT(*) FROM recourse_student WHERE status = 'active'";
        $active_stmt = $pdo->prepare($active_query);
        $active_stmt->execute();
    }
    $active_recourses = $active_stmt->fetchColumn();
    
    // Completed count
    if (!empty($stats_conditions)) {
        $completed_query = "SELECT COUNT(*) FROM recourse_student WHERE status = 'completed' AND " . implode(" AND ", $stats_conditions);
        $completed_stmt = $pdo->prepare($completed_query);
        $completed_stmt->execute($stats_completed_params);
    } else {
        $completed_query = "SELECT COUNT(*) FROM recourse_student WHERE status = 'completed'";
        $completed_stmt = $pdo->prepare($completed_query);
        $completed_stmt->execute();
    }
    $completed_recourses = $completed_stmt->fetchColumn();
    
    // Cancelled count
    if (!empty($stats_conditions)) {
        $cancelled_query = "SELECT COUNT(*) FROM recourse_student WHERE status = 'cancelled' AND " . implode(" AND ", $stats_conditions);
        $cancelled_stmt = $pdo->prepare($cancelled_query);
        $cancelled_stmt->execute($stats_cancelled_params);
    } else {
        $cancelled_query = "SELECT COUNT(*) FROM recourse_student WHERE status = 'cancelled'";
        $cancelled_stmt = $pdo->prepare($cancelled_query);
        $cancelled_stmt->execute();
    }
    $cancelled_recourses = $cancelled_stmt->fetchColumn();
}
...
<head>
    ...
    * {
    ...
    .role-badge.faculty-admin {
        background: linear-gradient(135deg, #00843D, #006a31);
        color: white;
    }

    .role-badge.department-admin {
        background: linear-gradient(135deg, #FF9800, #F57C00);
        color: white;
    }
...
<body>
                ...
                <div class="grid">
                    ...
                    <div>
                        ...
                        <select name="filter_campus" id="filter_campus" onchange="onFilterCampusChange(this.value)" 
                            <?= ($user_role !== 'super_admin') ? 'disabled' : '' ?>>
                    ...
                    <div>
                        ...
                        <select name="filter_faculty" id="filter_faculty" onchange="onFilterFacultyChange(this.value)" 
                            <?= ($user_role === 'faculty_admin' || $user_role === 'department_admin') ? 'disabled' : '' ?>>
                        ...
                        <?php if ($user_role === 'faculty_admin' || $user_role === 'department_admin'): ?>
                        <input type="hidden" name="filter_faculty" value="<?= $filter_faculty ?>">
                        <?php endif; ?>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="filter_department" id="filter_department" onchange="onFilterDepartmentChange(this.value)" 
                    ...
                    <div>
                        ...
                        <select name="filter_program" id="filter_program" onchange="onFilterProgramChange(this.value)">
                ...
                <form method="POST" id="recourseForm" onsubmit="return validateForm(this)">
                    ...
                    <div class="grid">
                        ...
                        <div class="hierarchy-section">
                            ...
                            <div class="hierarchy-grid">
                                ...
                                <div>
                                    ...
                                    <?php if ($user_role !== 'super_admin'): ?>
                                    <input type="hidden" id="modal_campus_hidden" value="<?= $filter_campus ?>">
                                    <?php endif; ?>
                                </div>
                        ...
                        <div>
                            ...
                            <?php if ($user_role !== 'super_admin'): ?>
                            <input type="hidden" name="recourse_campus_id" value="<?= $filter_campus ?>">
                            <?php endif; ?>
                        </div>
    ...
    const userCampusId = <?= $user_campus_id ?: 'null' ?>;
    const userFacultyId = <?= $user_faculty_id ?: 'null' ?>;
    const userDepartmentId = <?= $user_department_id ?: 'null' ?>;
    ...
    function openModal(id) {
        ...
        // Auto-select user's campus for non-super admins
        if (id === 'addModal' && userRole !== 'super_admin' && userCampusId) {
            const campusSelect = document.getElementById('modal_campus');
            if (campusSelect) {
                // Set the value and trigger change
                setTimeout(() => {
                    campusSelect.value = userCampusId;
                    onModalCampusChange(userCampusId);
                }, 100);
            }
            
            const recourseCampusSelect = document.getElementById('recourse_campus_id');
            if (recourseCampusSelect) {
                setTimeout(() => {
                    recourseCampusSelect.value = userCampusId;
    ...
    function onFilterCampusChange(campusId) {
        if (userRole !== 'super_admin') return; // Non-super admins can't change campus
        ...
        let url = `?ajax=get_faculties_by_campus&campus_id=${campusId}`;
        if (userFacultyId) url += `&faculty_id=${userFacultyId}`;
    ...
    function onFilterFacultyChange(facultyId) {
        if (userRole === 'faculty_admin' || userRole === 'department_admin') return;
        ...
        let url = `?ajax=get_departments_by_faculty&faculty_id=${facultyId}&campus_id=${campusId}`;
        if (userDepartmentId) url += `&department_id=${userDepartmentId}`;
    ...
    function onFilterDepartmentChange(departmentId) {
        if (userRole === 'department_admin') return;
    ...
    function resetFilter() {
        let url = 'recourse.php';
        // Preserve role-based filters
        if (userRole !== 'super_admin' && userCampusId) {
            url += `?filter_campus=${userCampusId}`;
            if (userRole === 'faculty_admin' && userFacultyId) {
                url += `&filter_faculty=${userFacultyId}`;
            }
            if (userRole === 'department_admin' && userDepartmentId) {
                url += `&filter_department=${userDepartmentId}`;
            }
        }
    ...
    function selectStudent(student) {
        ...
        // Auto-fill recourse campus with original campus if not set
                ...
                const html = `
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 25px;">
                        ...
                        <div>
                            ...
                            <p><strong>Registration No:</strong> ${recourse.reg_no || ''}</p>
                    ...
                    <div style="margin-top: 30px; text-align: center; display: flex; gap: 15px; justify-content: center;">
                        <button class="btn blue" onclick="closeModal('viewModal'); editRecourse(${JSON.stringify(recourse)})">
                            <i class="fas fa-edit"></i> Edit Record
                        </button>
                        <button class="btn gray" onclick="closeModal('viewModal')">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize filter dropdowns based on user role
        if (userRole === 'super_admin') {
            // Super admin can see all
            if (document.getElementById('filter_campus').value) {
                onFilterCampusChange(document.getElementById('filter_campus').value);
            }
        } else if (userRole === 'campus_admin' && userCampusId) {
            // Campus admin - auto-load their campus data
            onFilterCampusChange(userCampusId);
        } else if (userRole === 'faculty_admin' && userFacultyId) {
            // Faculty admin - load their faculty data
            if (document.getElementById('filter_faculty').value) {
                onFilterFacultyChange(userFacultyId);
            }
        } else if (userRole === 'department_admin' && userDepartmentId) {
            // Department admin - load their department data
            if (document.getElementById('filter_department').value) {
                onFilterDepartmentChange(userDepartmentId);
            }
        }
    });
        ...
        if (addModal) {
            ...
            openModal = function(id) {
                if (id === 'addModal') {
                    ...
                    // Auto-select campus based on user role
                    if (userRole !== 'super_admin' && userCampusId) {
                        setTimeout(() => {
                            const modalCampus = document.getElementById('modal_campus');
                            if (modalCampus) {
                                modalCampus.value = userCampusId;
                                onModalCampusChange(userCampusId);
                            }
                            
                            const recourseCampus = document.getElementById('recourse_campus_id');
                            if (recourseCampus) {
                                recourseCampus.value = userCampusId;
                                onRecourseCampusChange();
                            }
                        }, 100);
                    }
                    
                    // Auto-select faculty for faculty admin
                    if (userRole === 'faculty_admin' && userFacultyId) {
                        setTimeout(() => {
                            const modalFaculty = document.getElementById('modal_faculty');
                            if (modalFaculty && !modalFaculty.disabled) {
                                setTimeout(() => {
                                    modalFaculty.value = userFacultyId;
                                    onModalFacultyChange(userFacultyId);
                                }, 500);
                            }
                        }, 500);
                    }
                    
                    // Auto-select department for department admin
                    if (userRole === 'department_admin' && userDepartmentId) {
                        setTimeout(() => {
                            const modalDepartment = document.getElementById('modal_department');
                            if (modalDepartment && !modalDepartment.disabled) {
                                setTimeout(() => {
                                    modalDepartment.value = userDepartmentId;
                                    onModalDepartmentChange(userDepartmentId);
                                }, 1000);
                            }
                        }, 1000);
                    }
