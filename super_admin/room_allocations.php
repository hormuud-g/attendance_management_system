...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_allocation') {
        try {
            ...
            // Get room's details and validate capacity
            $stmt = $pdo->prepare("SELECT campus_id, department_id, capacity FROM rooms WHERE room_id = ?");
            $stmt->execute([$room_id]);
            $roomData = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if (empty($campus_id)) $campus_id = $roomData['campus_id'];
            if (empty($department_id)) $department_id = $roomData['department_id'];
    ...
    if ($action === 'update_allocation') {
        try {
            ...
            $stmt->execute([
                $campus_id,
                $academic_term_id, $department_id, $allocated_to,
                $start_date, $end_date, $remarks, $student_count,
    ...
    if ($action === 'delete_allocation') {
        try {
            ...
            $stmt = $pdo->prepare("SELECT room_id FROM room_allocation WHERE allocation_id = ?");
...
$departments = $pdo->query("
    SELECT department_id, department_name, department_code, faculty_id, campus_id 
    FROM departments 
    ORDER BY department_name
")->fetchAll(PDO::FETCH_ASSOC);

// Fetch all rooms for filter display
$allRooms = $pdo->query("
    ...
    WHERE r.status != 'inactive'
    ORDER BY c.campus_name, f.faculty_name, d.department_name, r.building_name, r.room_name
")->fetchAll(PDO::FETCH_ASSOC);
...
// Build main query for allocations
$query = "
    SELECT 
        ra.*,
        r.room_name,
...
// Stats
$total_allocations = $pdo->query("SELECT COUNT(*) FROM room_allocation")->fetchColumn();
$active_allocations = $pdo->query("
    SELECT COUNT(*) FROM room_allocation 
    WHERE status = 'active' 
    AND start_date <= CURDATE() 
    AND (end_date >= CURDATE() OR end_date IS NULL)
")->fetchColumn();
$upcoming_allocations = $pdo->query("
    SELECT COUNT(*) FROM room_allocation 
...
$class_allocations = $pdo->query("SELECT COUNT(*) FROM room_allocation WHERE allocated_to = 'Class'")->fetchColumn();
$exam_allocations = $pdo->query("SELECT COUNT(*) FROM room_allocation WHERE allocated_to = 'Exam'")->fetchColumn();
$maintenance_allocations = $pdo->query("SELECT COUNT(*) FROM room_allocation WHERE allocated_to = 'Maintenance'")->fetchColumn();
...
<head>
<meta charset="UTF-8">
<title>Room Allocation Management | Hormuud University</title>
...
/* =====================================================
   ...
   ===================================================== */
:root {
  --hormuud-green: #00A859;      /* Primary Green */
  --hormuud-dark-green: #00843D; /* Dark Green */
  --hormuud-light-green: #4CAF50; /* Light Green */
  --hormuud-blue: #0072CE;       /* Secondary Blue */
  --hormuud-gold: #FFB81C;       /* Gold/Yellow */
  --hormuud-orange: #F57C00;     /* Orange */
  --hormuud-red: #C62828;        /* Red/Danger */
  --hormuud-gray: #F5F5F5;       /* Light Gray */
  --hormuud-dark: #2C3E50;       /* Dark Blue Gray */
  --hormuud-white: #FFFFFF;      /* White */
  --hormuud-black: #212121;      /* Black */
}
...
<div class="main-content">
  <div class="page-header">
    <h1><i class="fas fa-calendar-alt"></i> Room Allocation Management</h1>
    <div style="display: flex; gap: 15px;">
      <a href="room_management.php" class="back-btn">
        <i class="fas fa-door-closed"></i> Back to Rooms
      </a>
        ...
        <div style="position:relative;">
          ...
          <select id="room" name="room" class="filter-input">
            ...
            foreach($allRooms as $room): 
              if ($room['campus_name'] != $currentCampus) {
                if ($currentCampus !== null) {
                  if ($currentFaculty !== null) echo '</optgroup>';
                  echo '</optgroup>';
                }
                $currentCampus = $room['campus_name'];
                $currentFaculty = null;
                echo '<optgroup label="🏛️ ' . htmlspecialchars($currentCampus ?? 'No Campus') . '">';
              }
              
              if ($room['faculty_name'] != $currentFaculty) {
                if ($currentFaculty !== null) echo '</optgroup>';
                $currentFaculty = $room['faculty_name'];
                echo '<optgroup label="&nbsp;&nbsp;📚 ' . htmlspecialchars($currentFaculty ?? 'No Faculty') . '">';
              }
              
              if ($room['department_name'] != $currentDept) {
                $currentDept = $room['department_name'];
            ...
            ?>
              <option value="<?= $room['room_id'] ?>" 
                ...
                <span style="color: <?= ($room['capacity'] > 0) ? 'var(--hormuud-green)' : 'var(--hormuud-red)'; ?>; font-weight: 600;">
                  [Cap: <?= $room['capacity'] ?>]
                </span>
              </option>
            <?php 
            ...
            if ($currentFaculty !== null) echo '</optgroup>';
            if ($currentCampus !== null) echo '</optgroup>';
            ?>
          </select>
        </div>
      </div>
              ...
              <td>
                <div style="display: flex; flex-direction: column; gap: 5px;">
                  ...
                  <div style="font-size: 11px;">
                    <?php if($a['student_count'] > $a['room_capacity']): ?>
                      <span style="color: var(--hormuud-red); font-weight: bold;">
                        <i class="fas fa-exclamation-triangle"></i> Exceeds by <?= $a['student_count'] - $a['room_capacity'] ?>
                      </span>
                    <?php else: ?>
                      <span style="color: var(--hormuud-green);">
                        <i class="fas fa-check-circle"></i> <?= $a['room_capacity'] - $a['student_count'] ?> available
                      </span>
                    <?php endif; ?>
                  </div>
                </div>
    ...
    <form method="POST" id="addAllocationForm" onsubmit="return validateAllocationForm('add')">
      ...
      <div class="form-grid">
        ...
        <div class="selection-group">
          ...
          <select id="add_campus" name="campus_id" class="form-control" required onchange="loadFaculties('add', this.value)">
        ...
        <div class="selection-group">
          ...
          <select id="add_faculty" name="faculty_id" class="form-control" required disabled onchange="loadDepartments('add', this.value)">
        ...
        <div class="form-group">
          ...
          <input type="number" id="add_student_count" name="student_count" class="form-control" min="0" oninput="validateStudentCount('add')">
  ...
  <div class="modal-content">
    ...
    <form method="POST" id="editAllocationForm" onsubmit="return validateAllocationForm('edit')">
      ...
      <div class="form-grid">
        ...
        <div class="selection-group">
          <label><i class="fas fa-map-marker-alt"></i> Campus *</label>
          <select id="edit_campus" name="campus_id" class="form-control" required onchange="loadFaculties('edit', this.value)">
            <option value="">-- Select Campus --</option>
            <?php foreach($campuses as $c): ?>
              <option value="<?= $c['campus_id'] ?>">
                <?= htmlspecialchars($c['campus_name']) ?> (<?= htmlspecialchars($c['campus_code']) ?>)
        ...
        <div class="selection-group">
          <label><i class="fas fa-university"></i> Faculty *</label>
          <select id="edit_faculty" name="faculty_id" class="form-control" required disabled onchange="loadDepartments('edit', this.value)">
            <option value="">-- Select Campus First --</option>
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="number" id="edit_student_count" name="student_count" class="form-control" min="0" oninput="validateStudentCount('edit')">
...
function validateStudentCount(prefix) {
    ...
    // Clear error if no room selected
    if (!roomSelect.value) {
        if (errorElement) {
...
// Load rooms by campus, faculty, department
    ...
    fetch(`?ajax=get_rooms&campus_id=${campusId}&faculty_id=${facultyId}&department_id=${departmentId}`)
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                // Update capacity info and validate after loading
...
function openEditAllocationModal(id, roomId, campusId, facultyId, deptId, termId, allocatedTo, startDate, endDate, remarks, studentCount, status, roomCapacity) {
    ...
    document.getElementById('edit_student_count').value = studentCount;
