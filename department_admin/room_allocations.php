...
$linked_table = $user['linked_table'] ?? '';
...
switch($role) {
    ...
    case 'faculty_admin':
        ...
        $access_id = $linked_id;
        // Get faculty's campus
        $stmt = $pdo->prepare("SELECT campus_id FROM faculties WHERE faculty_id = ?");
...
// ✅ For non-super admins, verify they have proper linked data
if ($role !== 'super_admin' && !$access_id) {
    header("Location: ../unauthorized.php");
    exit;
}
...
// AJAX: Get rooms by campus, faculty, department
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_rooms') {
    header('Content-Type: application/json');
    
    try {
        $req_campus_id = $_GET['campus_id'] ?? null;
        ...
        if ($role === 'campus_admin' && $campus_id) {
            $sql .= " AND r.campus_id = ?";
            $params[] = $campus_id;
        } elseif ($role === 'faculty_admin' && $faculty_id) {
            $sql .= " AND r.faculty_id = ?";
            $params[] = $faculty_id;
        } elseif ($role === 'department_admin' && $department_id) {
            $sql .= " AND r.department_id = ?";
            $params[] = $department_id;
        }
        
        if (!empty($req_campus_id) && $role === 'super_admin') {
            $sql .= " AND r.campus_id = ?";
            $params[] = $req_campus_id;
        }
        
        if (!empty($req_faculty_id) && $role === 'super_admin') {
            $sql .= " AND r.faculty_id = ?";
            $params[] = $req_faculty_id;
        }
        
        if (!empty($req_department_id) && $role === 'super_admin') {
            $sql .= " AND r.department_id = ?";
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_faculties_by_campus') {
    ...
    try {
        ...
        if ($role === 'campus_admin' && $campus_id) {
            $req_campus_id = $campus_id;
        } elseif ($role === 'faculty_admin' && $faculty_id) {
            $stmt = $pdo->prepare("SELECT * FROM faculties WHERE faculty_id = ? ORDER BY faculty_name");
            ...
            exit;
        } elseif ($role === 'department_admin' && $faculty_id) {
            $stmt = $pdo->prepare("SELECT * FROM faculties WHERE faculty_id = ? ORDER BY faculty_name");
            $stmt->execute([$faculty_id]);
            $faculties = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// AJAX: Get departments by campus and faculty
...
// AJAX handler for getting allocation details
...
// AJAX handler for getting room allocations
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_allocation') {
        try {
            ...
            if ($role === 'department_admin' && $dept_id != $department_id) {
            ...
            if ($role === 'faculty_admin') {
                ...
                $dept_faculty = $check->fetchColumn();
                if ($dept_faculty != $faculty_id) {
                    throw new Exception("You don't have permission to create allocations for this department");
                }
            }
            ...
            $campus_id_val = $_POST['campus_id'] ?? $campus_id;
            $academic_term_id = $_POST['academic_term_id'];
            $department_id_val = $_POST['department_id'] ?? $department_id;
            ...
            $stmt = $pdo->prepare("SELECT campus_id, department_id, capacity FROM rooms WHERE room_id = ?");
            $stmt->execute([$room_id]);
            ...
            $stmt->execute([
                $room_id, $campus_id_val, $academic_term_id, $department_id_val, $allocated_to,
    ...
    if ($action === 'update_allocation') {
        try {
            ...
            $check = $pdo->prepare("SELECT department_id FROM room_allocation WHERE allocation_id = ?");
            $check->execute([$allocation_id]);
            $alloc_dept = $check->fetchColumn();
            
            if ($role === 'department_admin' && $alloc_dept != $department_id) {
                throw new Exception("You don't have permission to update this allocation");
            }
            
            if ($role === 'faculty_admin') {
                $check = $pdo->prepare("SELECT faculty_id FROM departments WHERE department_id = ?");
                $check->execute([$alloc_dept]);
                $dept_faculty = $check->fetchColumn();
                if ($dept_faculty != $faculty_id) {
                    throw new Exception("You don't have permission to update this allocation");
                }
    ...
    if ($action === 'delete_allocation') {
        try {
            ...
            $check = $pdo->prepare("SELECT department_id, room_id FROM room_allocation WHERE allocation_id = ?");
            $check->execute([$allocation_id]);
            ...
            if ($role === 'department_admin' && $alloc['department_id'] != $department_id) {
                throw new Exception("You don't have permission to delete this allocation");
            }
            
            if ($role === 'faculty_admin') {
                $check = $pdo->prepare("SELECT faculty_id FROM departments WHERE department_id = ?");
                $check->execute([$alloc['department_id']]);
                $dept_faculty = $check->fetchColumn();
                if ($dept_faculty != $faculty_id) {
                    throw new Exception("You don't have permission to delete this allocation");
                }
...
// Fetch campuses (filtered by access)
$campuses = [];
if ($role === 'super_admin') {
    $campuses = $pdo->query("SELECT campus_id, campus_name, campus_code FROM campus ORDER BY campus_name")->fetchAll(PDO::FETCH_ASSOC);
} elseif ($role === 'campus_admin' && $campus_id) {
    $stmt = $pdo->prepare("SELECT campus_id, campus_name, campus_code FROM campus WHERE campus_id = ?");
    $stmt->execute([$campus_id]);
    $campuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
} elseif ($role === 'faculty_admin' && $campus_id) {
    $stmt = $pdo->prepare("SELECT campus_id, campus_name, campus_code FROM campus WHERE campus_id = ?");
    $stmt->execute([$campus_id]);
    $campuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
} elseif ($role === 'department_admin' && $campus_id) {
    $stmt = $pdo->prepare("SELECT campus_id, campus_name, campus_code FROM campus WHERE campus_id = ?");
    $stmt->execute([$campus_id]);
    $campuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// Fetch faculties (filtered by access)
$faculties = [];
if ($role === 'super_admin') {
    $faculties = $pdo->query("SELECT faculty_id, faculty_name, faculty_code FROM faculties ORDER BY faculty_name")->fetchAll(PDO::FETCH_ASSOC);
} elseif ($role === 'campus_admin' && $campus_id) {
    $stmt = $pdo->prepare("
        SELECT DISTINCT f.* 
        FROM faculties f
    ...
    $stmt = $pdo->prepare("SELECT faculty_id, faculty_name, faculty_code FROM faculties WHERE faculty_id = ?");
    $stmt->execute([$faculty_id]);
    $faculties = $stmt->fetchAll(PDO::FETCH_ASSOC);
} elseif ($role === 'department_admin' && $faculty_id) {
    $stmt = $pdo->prepare("SELECT faculty_id, faculty_name, faculty_code FROM faculties WHERE faculty_id = ?");
...
// Fetch departments (filtered by access)
$departments = [];
$deptQuery = "SELECT department_id, department_name, department_code, faculty_id, campus_id FROM departments WHERE 1=1";
$deptParams = [];

if ($role === 'campus_admin' && $campus_id) {
    $deptQuery .= " AND campus_id = ?";
    $deptParams[] = $campus_id;
} elseif ($role === 'faculty_admin' && $faculty_id) {
    $deptQuery .= " AND faculty_id = ?";
    $deptParams[] = $faculty_id;
} elseif ($role === 'department_admin' && $department_id) {
    $deptQuery .= " AND department_id = ?";
    $deptParams[] = $department_id;
}
...
// Fetch all rooms for filter display (filtered by access)
$roomQuery = "
...
if ($role === 'campus_admin' && $campus_id) {
    $roomQuery .= " AND r.campus_id = ?";
    $roomParams[] = $campus_id;
} elseif ($role === 'faculty_admin' && $faculty_id) {
    $roomQuery .= " AND r.faculty_id = ?";
    $roomParams[] = $faculty_id;
} elseif ($role === 'department_admin' && $department_id) {
    $roomQuery .= " AND r.department_id = ?";
    $roomParams[] = $department_id;
}

$roomQuery .= " ORDER BY c.campus_name, f.faculty_name, d.department_name, r.building_name, r.room_name";
...
// Build main query for allocations (with access restrictions)
$query = "
    SELECT 
        ra.*,
        r.room_name,
...
if ($role === 'campus_admin' && $campus_id) {
    $query .= " AND ra.campus_id = ?";
    $params[] = $campus_id;
} elseif ($role === 'faculty_admin' && $faculty_id) {
    $query .= " AND d.faculty_id = ?";
    $params[] = $faculty_id;
} elseif ($role === 'department_admin' && $department_id) {
    $query .= " AND ra.department_id = ?";
    $params[] = $department_id;
}
...
if (!empty($campus_filter) && $role === 'super_admin') {
    $query .= " AND ra.campus_id = ?";
    $params[] = $campus_filter;
}

if (!empty($room_filter) && $role === 'super_admin') {
    $query .= " AND ra.room_id = ?";
    $params[] = $room_filter;
} elseif (!empty($room_filter) && in_array($room_filter, array_column($allRooms, 'room_id'))) {
    $query .= " AND ra.room_id = ?";
    $params[] = $room_filter;
}
...
// Process allocations to ensure status is set
$current_date = date('Y-m-d');
...
// Stats (filtered by access)
$statsQuery = "SELECT COUNT(*) FROM room_allocation ra WHERE 1=1";
$statsParams = [];

if ($role === 'campus_admin' && $campus_id) {
    $statsQuery .= " AND ra.campus_id = ?";
    $statsParams[] = $campus_id;
} elseif ($role === 'faculty_admin' && $faculty_id) {
    $statsQuery .= " AND ra.department_id IN (SELECT department_id FROM departments WHERE faculty_id = ?)";
    $statsParams[] = $faculty_id;
} elseif ($role === 'department_admin' && $department_id) {
    $statsQuery .= " AND ra.department_id = ?";
    $statsParams[] = $department_id;
}
...
$location_context = '';
if ($role === 'campus_admin' && !empty($campuses)) {
    $location_context = ' - ' . ($campuses[0]['campus_name'] ?? '');
} elseif ($role === 'faculty_admin' && !empty($faculties)) {
    $location_context = ' - ' . ($faculties[0]['faculty_name'] ?? '');
} elseif ($role === 'department_admin' && !empty($departments)) {
    $location_context = ' - ' . ($departments[0]['department_name'] ?? '');
}
...
<head>
<meta charset="UTF-8">
<title>Room Allocation Management <?= htmlspecialchars($location_context) ?> | Hormuud University</title>
...
.location-context {
  ...
  gap: 15px;
  flex-wrap: wrap;
  border-left: 4px solid var(--hormuud-blue);
}
...
<div class="main-content">
  <div class="page-header">
    <h1>
      ...
      <!-- <span class="role-badge">
        <i class="fas fa-user-shield"></i> <?= htmlspecialchars($role_display) ?><?= htmlspecialchars($location_context) ?>
      </span> -->
    </h1>
  ...
  <div class="stats-container">
    <div class="stat-card total">
      ...
      <div class="stat-info">
        ...
        <div class="number"><?= $total_allocations ?></div>
      </div>
    </div>
    <div class="stat-card active">
      ...
      <div class="stat-info">
        ...
        <div class="number"><?= $active_allocations ?></div>
      </div>
    </div>
    <div class="stat-card upcoming">
      ...
      <div class="stat-info">
        <h3>Upcoming</h3>
        <div class="number"><?= $upcoming_allocations ?></div>
      </div>
    </div>
    <div class="stat-card past">
      <div class="stat-icon past"><i class="fas fa-history"></i></div>
      <div class="stat-info">
        <h3>Past Allocations</h3>
        <div class="number"><?= $past_allocations ?></div>
      </div>
    </div>
  </div>
  ...
  <div class="stats-container">
    <div class="stat-card class">
      <div class="stat-icon class"><i class="fas fa-chalkboard-teacher"></i></div>
      <div class="stat-info">
        <h3>Class Allocations</h3>
        <div class="number"><?= $class_allocations ?></div>
      </div>
    </div>
    <div class="stat-card exam">
      <div class="stat-icon exam"><i class="fas fa-file-alt"></i></div>
      <div class="stat-info">
        <h3>Exam Allocations</h3>
        <div class="number"><?= $exam_allocations ?></div>
      </div>
    </div>
    <div class="stat-card maintenance">
      ...
      <div class="stat-info">
        <h3>Maintenance</h3>
        <div class="number"><?= $maintenance_allocations ?></div>
      </div>
    </div>
  </div>
  ...
  <div class="filters-container">
    <div class="filter-header">
      <h3><i class="fas fa-filter"></i> Filter Allocations</h3>
    </div>
      ...
      <div class="filter-group">
        ...
        <div style="position:relative;">
          ...
          <select id="campus" name="campus" class="filter-input" 
            <?= ($role !== 'super_admin' && count($campuses) <= 1) ? 'disabled' : '' ?>>
  ...
  <div class="table-wrapper">
    <div class="table-header">
      <h3><i class="fas fa-list"></i> Allocations List</h3>
        ...
        <tbody>
          <?php if($allocations): ?>
            ...
            <?php 
              ...
              $can_edit = true;
              if ($role === 'department_admin' && $a['department_id'] != $department_id) {
                $can_edit = false;
              } elseif ($role === 'faculty_admin') {
                $dept_faculty = null;
                foreach ($departments as $dept) {
                  if ($dept['department_id'] == $a['department_id']) {
                    $dept_faculty = $dept['faculty_id'];
                    break;
                  }
                }
                if ($dept_faculty != $faculty_id) {
              ...
              } elseif ($role === 'campus_admin' && $a['campus_id'] != $campus_id) {
                $can_edit = false;
              }
            ?>
            <tr>
              ...
              <td>
                <div style="font-size: 12px; line-height: 1.6;">
                  <span style="color: var(--hormuud-blue); font-weight: 600;">
                    <i class="fas fa-map-marker-alt"></i> <?= htmlspecialchars($a['campus_name'] ?? 'N/A') ?>
                  </span><br>
            ...
            <tr>
              <td colspan="11">
                <div class="empty-state">
                  ...
                  <p>
                    <?php if(!empty($search) || !empty($campus_filter) || !empty($room_filter) || !empty($term_filter) || !empty($status_filter) || !empty($date_from) || !empty($date_to)): ?>
...
<div class="modal" id="addAllocationModal">
  <div class="modal-content">
    ...
    <h2><i class="fas fa-plus-circle"></i> Create Room Allocation</h2>
      ...
      <div class="form-grid">
        <div class="selection-group">
          ...
          <select id="add_campus" name="campus_id" class="form-control" required 
            <?= ($role !== 'super_admin' && $campus_id) ? 'disabled' : '' ?> 
            onchange="loadFaculties('add', this.value)">
            <option value="">-- Select Campus --</option>
            <?php foreach($campuses as $c): ?>
              <option value="<?= $c['campus_id'] ?>" 
                <?= ($campus_id == $c['campus_id'] && $role !== 'super_admin') ? 'selected' : '' ?>>
                <?= htmlspecialchars($c['campus_name']) ?> (<?= htmlspecialchars($c['campus_code']) ?>)
              </option>
            <?php endforeach; ?>
          </select>
          <?php if ($role !== 'super_admin' && $campus_id): ?>
        ...
        <div class="selection-group">
          ...
          <select id="add_faculty" name="faculty_id" class="form-control" required 
            <?= ($role !== 'super_admin' && $faculty_id) ? 'disabled' : '' ?> 
            onchange="loadDepartments('add', this.value)">
            <option value="">-- Select Campus First --</option>
            <?php if ($role !== 'super_admin' && $faculty_id): ?>
        ...
        <div class="selection-group">
          ...
          <select id="add_department" name="department_id" class="form-control" required 
            <?= ($role !== 'super_admin' && $department_id) ? 'disabled' : '' ?> 
            onchange="loadRooms('add')">
            <option value="">-- Select Faculty First --</option>
            <?php if ($role !== 'super_admin' && $department_id): ?>
              <?php foreach($departments as $d): ?>
                <option value="<?= $d['department_id'] ?>" selected>
      ...
      <div class="form-grid">
        <div class="selection-group">
          ...
          <select id="edit_campus" name="campus_id" class="form-control" required 
            <?= ($role !== 'super_admin' && $campus_id) ? 'disabled' : '' ?> 
            onchange="loadFaculties('edit', this.value)">
            <option value="">-- Select Campus --</option>
            <?php foreach($campuses as $c): ?>
              <option value="<?= $c['campus_id'] ?>">
                <?= htmlspecialchars($c['campus_name']) ?> (<?= htmlspecialchars($c['campus_code']) ?>)
        ...
        <div class="selection-group">
          ...
          <select id="edit_faculty" name="faculty_id" class="form-control" required 
            <?= ($role !== 'super_admin' && $faculty_id) ? 'disabled' : '' ?> 
            onchange="loadDepartments('edit', this.value)">
            <option value="">-- Select Campus First --</option>
          </select>
        </div>
...
$(document).ready(function() {
    $('#campus, #room, #term, #status').select2({
    ...
    $('#campus, #room, #term, #status').on('change', function() {
    ...
    if (userRole !== 'super_admin') {
        if (campusId) {
            loadFaculties('add', campusId);
            if (facultyId) {
                setTimeout(() => {
                    document.getElementById('add_faculty').value = facultyId;
                    loadDepartments('add', facultyId);
                    
                    if (departmentId) {
                        setTimeout(() => {
                            document.getElementById('add_department').value = departmentId;
                            loadRooms('add');
                        }, 200);
                    }
                }, 200);
...
function loadFaculties(prefix, campusId) {
    ...
    if (!campusId) {
        facultySelect.innerHTML = '<option value="">-- Select Campus First --</option>';
    ...
    fetch(`?ajax=get_faculties_by_campus&campus_id=${campusId}`)
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                if (data.faculties.length === 1 && userRole !== 'super_admin') {
                    facultySelect.value = data.faculties[0].faculty_id;
                    loadDepartments(prefix, data.faculties[0].faculty_id);
                }
            } else {
...
function loadDepartments(prefix, facultyId) {
    ...
    fetch(`?ajax=get_departments&campus_id=${campusId}&faculty_id=${facultyId}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                ...
                if (data.departments.length === 1 && userRole === 'department_admin') {
                    deptSelect.value = data.departments[0].department_id;
                    loadRooms(prefix);
                }
            } else {
...
function loadRooms(prefix) {
    ...
    fetch(`?ajax=get_rooms&campus_id=${campusId}&faculty_id=${facultyId}&department_id=${departmentId}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
...
function closeModal(id) {
    ...
    if (modal) {
        ...
        if (id === 'addAllocationModal') {
            ...
            document.getElementById('add_faculty').innerHTML = '<option value="">-- Select Campus First --</option>';
            ...
            if (userRole !== 'super_admin' && campusId) {
                setTimeout(() => {
                    document.getElementById('add_campus').value = campusId;
                    loadFaculties('add', campusId);
...
function openEditAllocationModal(id, roomId, campusId, facultyId, deptId, termId, allocatedTo, startDate, endDate, remarks, studentCount, status, roomCapacity) {
    ...
    document.getElementById('edit_student_count').value = studentCount || 0; // Ensure student count is set
    ...
    for(let i = 0; i < statusSelect.options.length; i++) {
        if(statusSelect.options[i].value === statusSelect.value) {
            found = true;
            console.log('Option found:', statusSelect.options[i].text);
            break;
        }
    }
    if(!found) {
        console.error('❌ Status option not found in select!');
        statusSelect.selectedIndex = 0;
        console.log('Defaulted to:', statusSelect.options[0].text);
    }
