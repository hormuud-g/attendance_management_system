...
// Add faculty restriction for faculty admin
if ($is_faculty_admin && $current_user_linked_id) {
    // For faculty admin, they can see promotions where either old_faculty_id OR new_faculty_id is their faculty
    $where .= " AND (ph.old_faculty_id = ? OR ph.new_faculty_id = ?)";
...
// Old Faculty Filter (only for super admin)
if (!empty($_GET['old_faculty_id']) && !$is_faculty_admin) { 
    $where .= " AND ph.old_faculty_id=?"; 
    $params[] = $_GET['old_faculty_id']; 
}

// New Faculty Filter (only for super admin)
if (!empty($_GET['new_faculty_id']) && !$is_faculty_admin) { 
...
<div class="main-content">
  ...
  <div class="filter-box">
    <form method="GET">
      ...
      <div class="filter-section">
        <h3 style="color:#0072CE;margin-bottom:15px;">Old Information Filters</h3>
        <div class="grid">
          <div>
            <label>Old Campus</label>
            <select name="old_campus_id" id="old_campus" onchange="loadOldFaculties()">
              <option value="">All Campuses</option>
              <?php foreach($campuses as $c): ?>
                <option value="<?= $c['campus_id'] ?>" <?= (!empty($_GET['old_campus_id']) && $_GET['old_campus_id']==$c['campus_id'])?'selected':'' ?>>
          ...
          <div>
            ...
            <?php if($is_faculty_admin): ?>
              ...
              <input type="hidden" name="old_faculty_id" value="<?= $current_user_linked_id ?>">
            <?php else: ?>
              <select name="old_faculty_id" id="old_faculty" onchange="loadOldDepartments()" disabled>
                <option value="">All Faculties</option>
                <?php if(!empty($_GET['old_faculty_id'])): ?>
                  <?php 
                  if(!empty($_GET['old_campus_id'])) {
                    $stmt = $pdo->prepare("
                      SELECT DISTINCT f.faculty_id, f.faculty_name 
          ...
          <div>
            ...
            <select name="old_department_id" id="old_department" onchange="loadOldPrograms()" disabled>
              <option value="">All Departments</option>
              <?php if(!empty($_GET['old_department_id']) && !empty($_GET['old_faculty_id']) && !empty($_GET['old_campus_id'])): ?>
                <?php 
                $stmt = $pdo->prepare("
                  SELECT department_id, department_name 
                  FROM departments 
      ...
      <div class="filter-section" style="margin-top:20px;">
        <h3 style="color:#0072CE;margin-bottom:15px;">New Information Filters</h3>
        <div class="grid">
          <div>
          ...
          <div>
            ...
            <?php if($is_faculty_admin): ?>
              <input type="text" class="form-control" value="<?= htmlspecialchars($faculty_name) ?> (Restricted)" readonly style="background:#f0f0f0;padding:8px;border:1px solid #ccc;border-radius:6px;">
              <input type="hidden" name="new_faculty_id" value="<?= $current_user_linked_id ?>">
            <?php else: ?>
  ...
  <div class="table-wrapper" id="reportArea">
    ...
    <?php if($promotions): ?>
    <div style="padding:10px 15px;text-align:right;color:#666;border-top:1px solid #eee;">
      Total Records: <?= count($promotions) ?>
    </div>
    <?php endif; ?>
...
function loadOldFaculties() {
    ...
    if (isFacultyAdmin) {
        ...
        loadOldDepartmentsDirectly(campusId);
        return;
    }
...
function loadOldDepartmentsDirectly(campusId) {
    if (!campusId) {
        resetOldHierarchy(['department', 'program']);
        return;
    }
...
function loadOldDepartments() {
    const facultyId = isFacultyAdmin ? facultyAdminId : document.getElementById('old_faculty').value;
    const campusId = document.getElementById('old_campus').value;
    const deptSelect = document.getElementById('old_department');
...
function loadNewFaculties() {
    ...
    if (isFacultyAdmin) {
        // For faculty admin, directly load departments
        loadNewDepartmentsDirectly(campusId);
        return;
    }
...
function loadNewDepartmentsDirectly(campusId) {
    if (!campusId) {
        resetNewHierarchy(['department', 'program']);
        return;
    }
...
function loadNewDepartments() {
    const facultyId = isFacultyAdmin ? facultyAdminId : document.getElementById('new_faculty').value;
    const campusId = document.getElementById('new_campus').value;
    const deptSelect = document.getElementById('new_department');
...
window.onload = function() {
    ...
    const oldCampus = document.getElementById('old_campus');
    
    if (oldCampus && oldCampus.value && isFacultyAdmin) {
        loadOldDepartmentsDirectly(oldCampus.value);
    }
    
    if (oldFaculty && oldFaculty.options.length > 1 && !isFacultyAdmin) {
        oldFaculty.disabled = false;
    }
    if (oldDepartment && oldDepartment.options.length > 1) {
        oldDepartment.disabled = false;
    }
    if (oldProgram && oldProgram.options.length > 1) {
        oldProgram.disabled = false;
    }
    ...
    const newCampus = document.getElementById('new_campus');
    
    if (newCampus && newCampus.value && isFacultyAdmin) {
        loadNewDepartmentsDirectly(newCampus.value);
    }
    
    if (newFaculty && newFaculty.options.length > 1 && !isFacultyAdmin) {
        newFaculty.disabled = false;
    }
    if (newDepartment && newDepartment.options.length > 1) {
        newDepartment.disabled = false;
    }
    if (newProgram && newProgram.options.length > 1) {
