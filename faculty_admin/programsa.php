...
// ✅ Auto set faculty_id from session
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if ($_POST['action'] === 'add') {
            ...
            $checkDept->execute([$department_id, $faculty_id]);
        ...
        if ($_POST['action'] === 'update') {
            // Validate required fields
            $required_fields = ['program_id', 'program_name', 'program_code', 'department_id', 'duration_years', 'status'];
            foreach ($required_fields as $field) {
                if (empty(trim($_POST[$field] ?? ''))) {
                    throw new Exception("All required fields must be filled!");
                }
...
// ✅ Programs for this faculty
$stmt = $pdo->prepare("SELECT p.*, f.faculty_name, d.department_name, d.department_id
    FROM programs p
...
// ✅ Departments under this faculty with campus name
...
<div class="main-content">
    ...
    <!-- <div class="info-banner">
        <i class="fas fa-university"></i> Managing programs for: <strong><?= htmlspecialchars($faculty_data['faculty_name']) ?></strong>
    </div> -->
    <?php endif; ?>
    ...
    <div class="table-wrapper">
        ...
        <div class="empty-state">
            ...
            <p>Get started by adding your first program to this faculty.</p>
        </div>
        <?php else: ?>
        <table>
            <thead>
        ...
        <form method="POST" id="addForm">
            ...
            <div class="form-group">
                ...
                <select id="add_department" name="department_id" required>
                    ...
                    <?php 
                    // Group departments by campus
                    $current_campus = '';
                    foreach($departments as $d): 
                        if ($current_campus != $d['campus_name']):
                            if ($current_campus != '') echo '</optgroup>';
                            echo '<optgroup label="' . htmlspecialchars($d['campus_name']) . ' Campus">';
                            $current_campus = $d['campus_name'];
                        endif;
                    ?>
                    ...
                    <?php endforeach; ?>
                    <?php if (!empty($departments)) echo '</optgroup>'; ?>
                </select>
                <small style="color: #666;">Departments are grouped by campus</small>
            </div>
