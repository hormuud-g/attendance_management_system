 ...
 * WITH DEPARTMENT ADMIN SUPPORT USING TIMETABLE FOR TEACHER-DEPARTMENT RELATIONSHIP
 */
...
// ✅ Access Control with Department Admin Support
...
function autoTimeoutTeachers($pdo, $isDepartmentAdmin = false, $restrictedDepartmentId = null) {
    date_default_timezone_set('Africa/Nairobi');
    $today = date('Y-m-d');
    $dayName = date('D', strtotime($today));
    
    // Build query with department restrictions using timetable
    $sql = "
        SELECT 
            tb.teacher_id, 
    ...
    if ($isDepartmentAdmin) {
        $sql .= " AND tb.department_id = " . intval($restrictedDepartmentId);
    }
    
    $sql .= " GROUP BY tb.teacher_id";
    
    $autoTeachers = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
...
// Run auto timeout on page load with department restrictions
$autoTimeoutCount = autoTimeoutTeachers($pdo, $isDepartmentAdmin, $restrictedDepartmentId);
...
if (isset($_GET['ajax'])) {
    ...
    // GET FACULTIES BY CAMPUS (with department admin restrictions)
    ...
    // GET DEPARTMENTS BY FACULTY & CAMPUS (with department admin restrictions)
    ...
    // GET TEACHERS BY DEPARTMENT - Using timetable to find teachers in a department
    if ($_GET['ajax'] == 'get_teachers_by_department') {
        $department_id = $_GET['department_id'] ?? 0;
        ...
        $stmt = $pdo->prepare("
            SELECT DISTINCT t.teacher_id, t.teacher_name, t.email, t.phone_number
            FROM teachers t
    ...
    if ($_GET['ajax'] == 'search_students') {
        ...
        $sql = "
            SELECT s.student_id, s.full_name, s.reg_no, s.email,
                   d.department_id, d.faculty_id, d.campus_id
            FROM students s
            LEFT JOIN student_enroll se ON s.student_id = se.student_id
            LEFT JOIN departments d ON se.department_id = d.department_id
        ...
        if ($isDepartmentAdmin) {
            $sql .= " AND (se.department_id = ? OR d.department_id = ?)";
            $params[] = $restrictedDepartmentId;
            $params[] = $restrictedDepartmentId;
        }
        
        $sql .= " ORDER BY s.full_name LIMIT 10";
    ...
    // AJAX for getting student attendance - Using timetable for department check
    if ($_GET['ajax'] == 'get_student_attendance') {
        ...
        $sql = "
            ...
            LEFT JOIN timetable tt ON a.teacher_id = tt.teacher_id AND a.class_id = tt.class_id AND a.subject_id = tt.subject_id
            WHERE a.student_id = ? 
        ...
        if ($isDepartmentAdmin) {
            $sql .= " AND tt.department_id = ?";
            $params[] = $restrictedDepartmentId;
        }
        
        $sql .= " GROUP BY a.attendance_id ORDER BY a.attendance_date DESC";
    ...
    // AJAX for approving correction - Check through timetable
    if ($_GET['ajax'] == 'approve_correction') {
        $leave_id = $_GET['correction_id'] ?? 0;
        $approved_by = $_SESSION['user']['user_id'] ?? 0;
        
        // Check if correction belongs to department admin's department
        if ($isDepartmentAdmin) {
            $check = $pdo->prepare("
                SELECT ac.leave_id 
                FROM attendance_correction ac
                JOIN timetable tt ON ac.teacher_id = tt.teacher_id AND ac.subject_id = tt.subject_id
                WHERE ac.leave_id = ? AND tt.department_id = ?
                GROUP BY ac.leave_id
            ");
            $check->execute([$leave_id, $restrictedDepartmentId]);
            if (!$check->fetch()) {
    ...
    // AJAX for rejecting correction - Check through timetable
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        // Check if teacher belongs to department admin's department using timetable
        if ($isDepartmentAdmin) {
            $checkTeacher = $pdo->prepare("
                SELECT tt.department_id 
                FROM timetable tt
                WHERE tt.teacher_id = ? 
                AND tt.department_id = ?
                LIMIT 1
            ");
            $checkTeacher->execute([$teacher_id, $restrictedDepartmentId]);
            $teacherDept = $checkTeacher->fetchColumn();
            
            if (!$teacherDept) {
                throw new Exception("You don't have permission to manage this teacher's attendance");
            }
        }
        ...
        if ($action === 'request_correction') {
            ...
            // Get teacher_id from attendance record and check department
            $teacherStmt = $pdo->prepare("
                SELECT a.teacher_id, tt.department_id 
                FROM attendance a
                LEFT JOIN timetable tt ON a.teacher_id = tt.teacher_id AND a.class_id = tt.class_id AND a.subject_id = tt.subject_id
                WHERE a.student_id = ? 
                AND a.subject_id = ? 
            ...
            // Check if teacher belongs to department admin's department
            if ($isDepartmentAdmin && $attendance['department_id'] != $restrictedDepartmentId) {
                throw new Exception("You can only request corrections for teachers in your department");
            }
...
// ============================================
// DATA FETCHING FOR DISPLAY (WITH DEPARTMENT RESTRICTIONS)
// ============================================

// ✅ Teachers (filtered by department for department admin using timetable)
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT DISTINCT t.teacher_id, t.teacher_name, t.email, t.phone_number
        FROM teachers t
    ...
    ");
    $stmt->execute([$restrictedDepartmentId]);
    $teachers = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
    $teachers = $pdo->query("SELECT teacher_id, teacher_name, email, phone_number FROM teachers WHERE status='active' ORDER BY teacher_name ASC")->fetchAll(PDO::FETCH_ASSOC);
}

// ✅ Campuses (filtered for department admin)
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("SELECT campus_id, campus_name FROM campus WHERE campus_id = ? AND status='active' ORDER BY campus_name ASC");
...
if (!empty($_POST['campus_id'])) {
    ...
    if ($isDepartmentAdmin) {
        $sql .= " AND f.faculty_id = ?";
        $params = [$_POST['campus_id'], $restrictedFacultyId];
    } else {
        $params = [$_POST['campus_id']];
    }
...
if (!empty($_POST['faculty_id']) && !empty($_POST['campus_id'])) {
    ...
    if ($isDepartmentAdmin) {
        $sql .= " AND department_id = ?";
        $params = [$_POST['faculty_id'], $_POST['campus_id'], $restrictedDepartmentId];
    } else {
        $params = [$_POST['faculty_id'], $_POST['campus_id']];
    }
...
if (!empty($_POST['department_id']) && !empty($_POST['faculty_id']) && !empty($_POST['campus_id'])) {
    ...
    if (!$isDepartmentAdmin || ($isDepartmentAdmin && $_POST['department_id'] == $restrictedDepartmentId)) {
        $stmt = $pdo->prepare("
            SELECT program_id, program_name 
            FROM programs 
            WHERE department_id = ? 
...
// ✅ Get Email Logs (filtered for department admin)
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT 
            el.log_id,
...
// ✅ Attendance Views (INCLUDING RECOURSE STUDENTS) - filtered for department admin using timetable
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT 
            a.attendance_date, 
...
// Get all recourse students (filtered for department admin)
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT 
            rs.recourse_id,
        ...
        WHERE od.department_id = ? OR rd.department_id = ?
        ORDER BY rs.created_at DESC
    ");
    $stmt->execute([$restrictedDepartmentId, $restrictedDepartmentId]);
...
// ✅ Teacher attendance view (filtered for department admin using timetable)
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT 
            ta.date, 
...
/* ================= FETCH PENDING CORRECTIONS (UPDATED WITH DEPARTMENT FILTER) ================= */
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT 
        ...
        LEFT JOIN timetable tt ON ac.teacher_id = tt.teacher_id AND ac.subject_id = tt.subject_id
        WHERE ac.status = 'pending' AND tt.department_id = ?
        GROUP BY ac.leave_id
...
/* ================= FETCH APPROVED CORRECTIONS (UPDATED WITH DEPARTMENT FILTER) ================= */
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT 
            ac.leave_id as correction_id,
        ...
        LEFT JOIN timetable tt ON ac.teacher_id = tt.teacher_id AND ac.subject_id = tt.subject_id
        WHERE ac.status = 'approved' AND tt.department_id = ?
        GROUP BY ac.leave_id
...
/* ================= FETCH REJECTED CORRECTIONS (UPDATED WITH DEPARTMENT FILTER) ================= */
if ($isDepartmentAdmin) {
    $stmt = $pdo->prepare("
        SELECT 
            ac.leave_id as correction_id,
        ...
        LEFT JOIN users u ON ac.requested_by = u.user_id
        LEFT JOIN timetable tt ON ac.teacher_id = tt.teacher_id AND ac.subject_id = tt.subject_id
        WHERE ac.status = 'rejected' AND tt.department_id = ?
        GROUP BY ac.leave_id
        ORDER BY ac.updated_at DESC
        LIMIT 20
    ");
...
if (isset($_GET['send_absence_emails']) && $_GET['send_absence_emails'] == 'now') {
    ...
    $params = [$today, $academic_term_id];
    
    if ($isDepartmentAdmin) {
        $sql .= " AND EXISTS (SELECT 1 FROM timetable tt WHERE tt.teacher_id = a.teacher_id AND tt.department_id = ?)";
        $params[] = $restrictedDepartmentId;
    }
    
    $sql .= " GROUP BY a.student_id, a.subject_id, a.academic_term_id";
...
<head>
    ...
    <title>Attendance Management - <?php echo $isDepartmentAdmin ? 'Department Admin' : 'Super Admin'; ?></title>
    ...
    <style>
        ...
        .page-header { margin-bottom: 25px; padding-bottom: 15px; border-bottom: 2px solid #0072CE; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; }
        .page-header h1 { color: #0072CE; font-weight: 600; font-size: 28px; margin: 0; }
        .page-header .badge { background: #00843D; color: white; padding: 8px 16px; border-radius: 20px; font-size: 14px; font-weight: 600; }
        ...
        .department-badge { background: #0072CE; color: white; padding: 5px 10px; border-radius: 5px; font-size: 13px; margin-left: 10px; }
        @keyframes fadeIn {
        ...
        @media (max-width: 768px) {
            .main-content { padding: 15px; margin-left: 0; }
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <?php if ($isDepartmentAdmin && !empty($departmentInfo)): ?>
                <span class="badge">
                    <i class="fas fa-building"></i> <?php echo htmlspecialchars($departmentInfo['department_name']); ?> Department
                    (<?php echo htmlspecialchars($departmentInfo['faculty_name']); ?>)
                </span>
            <?php endif; ?>
                ...
                <form method="POST" id="studentForm">
                    <div class="grid">
                        ...
                        <div>
                            ...
                            <select name="campus_id" id="campus_id" class="form-select" required <?php echo $isDepartmentAdmin ? 'disabled' : ''; ?>>
                            ...
                            <?php if ($isDepartmentAdmin): ?>
                                <input type="hidden" name="campus_id" value="<?php echo $restrictedCampusId; ?>">
                            <?php endif; ?>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="faculty_id" id="faculty_id" class="form-select" <?php echo $isDepartmentAdmin ? 'disabled' : ''; ?>>
                            ...
                            <?php if ($isDepartmentAdmin): ?>
                                <input type="hidden" name="faculty_id" value="<?php echo $restrictedFacultyId; ?>">
                            <?php endif; ?>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="department_id" id="department_id" class="form-select" <?php echo $isDepartmentAdmin ? 'disabled' : ''; ?>>
                            ...
                            <?php if ($isDepartmentAdmin): ?>
                                <input type="hidden" name="department_id" value="<?php echo $restrictedDepartmentId; ?>">
                            <?php endif; ?>
                        </div>
                            ...
                            <select name="teacher_id" id="teacher_id" class="form-select" required>
                                ...
                                <?php foreach ($teachers as $t): ?>
                                    <option value="<?php echo $t['teacher_id']; ?>" <?php echo ($_POST['teacher_id'] ?? '') == $t['teacher_id'] ? 'selected' : ''; ?>>
                                        ...
                                        <?php if ($isDepartmentAdmin): ?> (Your Department) <?php endif; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                ...
                <div class="table-wrapper">
                    <table>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="<?php echo $isDepartmentAdmin ? '9' : '10'; ?>" style="text-align:center; padding: 40px; color: #666;">
    ...
    $(document).ready(function() {
        ...
        const isDepartmentAdmin = <?php echo json_encode($isDepartmentAdmin); ?>;
        const restrictedCampusId = <?php echo json_encode($restrictedCampusId); ?>;
        const restrictedFacultyId = <?php echo json_encode($restrictedFacultyId); ?>;
        const restrictedDepartmentId = <?php echo json_encode($restrictedDepartmentId); ?>;
        ...
        $('#department_id').change(function() {
            ...
            // For department admin, use restricted values
            if (isDepartmentAdmin) {
                departmentId = restrictedDepartmentId;
        ...
        $('#program_id').change(function() {
            ...
            var departmentId = isDepartmentAdmin ? restrictedDepartmentId : $('#department_id').val();
            var facultyId = isDepartmentAdmin ? restrictedFacultyId : $('#faculty_id').val();
            var campusId = isDepartmentAdmin ? restrictedCampusId : $('#campus_id').val();
        ...
        function initializeHierarchy() {
            if (isDepartmentAdmin) {
                // For department admin, trigger department change directly
                $('#department_id').val(restrictedDepartmentId).trigger('change');
            } else {
                var campusId = $('#campus_id').val();
                if(campusId) {
