...
// Check if user has permission (super_admin or campus_admin)
$user_role = strtolower($_SESSION['user']['role'] ?? '');
$user_id = $_SESSION['user']['id'] ?? null;
$user_linked_id = $_SESSION['user']['linked_id'] ?? null;

if (!in_array($user_role, ['super_admin', 'campus_admin'])) {
...
// Get campus admin's assigned campus if applicable
$assigned_campus_id = null;
if ($user_role === 'campus_admin' && $user_linked_id) {
    // Verify the linked campus exists and is active
...
// ===========================================
if (isset($_GET['ajax'])) {
    // GET FACULTIES BY CAMPUS (with campus_admin restriction)
    if ($_GET['ajax'] == 'get_faculties_by_campus') {
        $campus_id = $_GET['campus_id'] ?? 0;
        
        // Check if campus_admin is trying to access another campus
    ...
    // GET DEPARTMENTS BY FACULTY & CAMPUS (with campus_admin restriction)
    ...
    // GET PROGRAMS BY DEPARTMENT, FACULTY & CAMPUS (with campus_admin restriction)
    ...
    // GET CLASSES BY PROGRAM, DEPARTMENT, FACULTY & CAMPUS (with campus_admin restriction)
    ...
    // GET STUDENTS BY CLASS AND CAMPUS (with campus_admin restriction)
    ...
    // GET SUBJECTS BY SEMESTER AND HIERARCHY (with campus_admin restriction)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['promote'])) {
    try {
        // First, check campus_admin restrictions
        if ($user_role === 'campus_admin') {
            $from_campus_id = $_POST['from_campus'] ?? null;
            $to_campus_id = $_POST['to_campus'] ?? null;
            
            // Campus admin can only promote within their assigned campus
            if ($from_campus_id != $assigned_campus_id || $to_campus_id != $assigned_campus_id) {
                throw new Exception("Access denied! You can only promote students within your assigned campus.");
            }
        }
        ...
        foreach ($student_ids as $student_id) {
            // Get current enrollment data (only active)
            $old = $pdo->prepare("
                SELECT se.*, 
                    c.campus_name,
                    f.faculty_name,
...
/* ✅ Campuses (ACTIVE ONLY) - restricted for campus_admin */
...
} else if ($user_role === 'campus_admin' && $assigned_campus_id) {
    // Campus admin only sees their assigned campus
    $stmt = $pdo->prepare("
        SELECT campus_id, campus_name 
        FROM campus 
        WHERE campus_id = ? 
...
/* ✅ Promotion History (restricted for campus_admin) */
if ($user_role === 'super_admin') {
    $history = $pdo->query("
        SELECT 
        ph.promotion_date,
...
<div class="main-content">
    <div class="page-header">
        ...
        <?php if ($user_role === 'campus_admin' && $assigned_campus_id): ?>
            <?php 
                $campus_name = $pdo->prepare("SELECT campus_name FROM campus WHERE campus_id = ?");
                $campus_name->execute([$assigned_campus_id]);
                $campus = $campus_name->fetch(PDO::FETCH_ASSOC);
            ?>
...
<div class="filter-box">
    <form method="POST" id="promotionForm">
        <!-- Hidden field to track from campus for campus_admin validation -->
        ...
        <div class="grid">
            ...
            <fieldset style="border:1px solid #ccc;border-radius:8px;padding:10px;">
                ...
                <div>
                    ...
                    <select id="from_campus" onchange="fromCampusChange()" required 
                        <?php if ($user_role === 'campus_admin'): ?>disabled<?php endif; ?>>
                        <option value="">Select Campus</option>
                        <?php foreach($campuses as $c): ?>
                            <option value="<?= $c['campus_id'] ?>" 
                                <?php if ($user_role === 'campus_admin' && $c['campus_id'] == $assigned_campus_id): ?>selected<?php endif; ?>>
                                <?= htmlspecialchars($c['campus_name']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <?php if ($user_role === 'campus_admin'): ?>
            ...
            <fieldset style="border:1px solid #ccc;border-radius:8px;padding:10px;">
                ...
                <div>
                    ...
                    <?php if ($user_role === 'campus_admin'): ?>
                        <input type="hidden" name="to_campus" value="<?= $assigned_campus_id ?>">
                    <?php endif; ?>
                </div>
    ...
    <div class="table-wrapper">
        ...
        <?php if (empty($history)): ?>
            <div class="restricted-info">
                <i class="fa fa-info-circle"></i> No promotion history found for your campus.
            </div>
        <?php else: ?>
            <table>
                <thead>
...
<script>
// Set initial campus for campus_admin
<?php if ($user_role === 'campus_admin' && $assigned_campus_id): ?>
    document.addEventListener('DOMContentLoaded', function() {
        // Trigger from campus change on page load for campus_admin
        ...
        // Trigger to campus change on page load for campus_admin
        const toCampusSelect = document.getElementById('to_campus');
        toCampusSelect.value = '<?= $assigned_campus_id ?>';
        toCampusChange();
    });
<?php endif; ?>
