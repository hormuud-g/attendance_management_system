...
/* ===========================================
   FETCH ALL ACADEMIC YEARS (read-only for faculty admin)
=========================================== */
try {
    $stmt = $pdo->query("
...
<head>
<meta charset="UTF-8">
<title>Academic Years | Faculty Admin - <?= htmlspecialchars($faculty_name) ?> | Hormuud University</title>
...
<div class="main-content">
  ...
  <div class="table-wrapper">
    <div class="table-header">
      <h3><i class="fas fa-list"></i> Academic Years List</h3>
      <div class="results-count">
        <i class="fas fa-eye"></i> Showing <?= count($years) ?> years
      </div>
    </div>
          ...
          <tr>
            ...
            <td>
              <span class="year-badge">
                <i class="fas fa-calendar"></i> <?= htmlspecialchars($y['year_name']) ?>
              </span>
            </td>
            <td>
              <i class="fas fa-play" style="color: var(--green); font-size: 12px; margin-right: 5px;"></i>
              <?= date('d M Y', strtotime($y['start_date'])) ?>
            </td>
            <td>
              <i class="fas fa-stop" style="color: var(--red); font-size: 12px; margin-right: 5px;"></i>
              <?= date('d M Y', strtotime($y['end_date'])) ?>
            </td>
            <td>
              <span class="status-<?= strtolower($y['status']) ?>">
                <i class="fas fa-<?= $y['status'] === 'active' ? 'check-circle' : 'pause-circle' ?>"></i>
                <?= ucfirst($y['status']) ?>
              </span>
            </td>
            <td>
              <span style="color: #666; font-size: 13px;">
                <i class="fas fa-clock"></i> <?= date('d M Y H:i', strtotime($y['created_at'] ?? 'now')) ?>
              </span>
            </td>
            <td>
              <span style="color: #666; font-size: 13px;">
                <i class="fas fa-sync-alt"></i> <?= date('d M Y H:i', strtotime($y['updated_at'] ?? 'now')) ?>
              </span>
            </td>
          </tr>
          <?php endforeach; ?>
          ...
          <tr>
            <td colspan="7">
              <div class="empty-state">
                ...
                <p>There are no academic years available at the moment.</p>
              </div>
            </td>
          </tr>
        <?php endif; ?>
