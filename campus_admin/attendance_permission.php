<?php
/**
 * ATTENDANCE CORRECTION & UNLOCK SYSTEM - CAMPUS ADMIN PANEL
 * Manages attendance correction requests and teacher unlock requests for their campus
 * Campus admins can only view/process requests from their assigned campus
 */
...
// ✅ Access Control - Campus Admin only
if (!isset($_SESSION['user']) || strtolower($_SESSION['user']['role']) !== 'campus_admin') {
    session_destroy();
...
$current_user_name = $_SESSION['user']['username'] ?? 'Campus Admin';

// Get campus admin's assigned campus
$campus_id = null;
$campus_name = '';

try {
    // Get the campus admin's linked_id (which should be the campus_id)
    if (!empty($_SESSION['user']['linked_id']) && $_SESSION['user']['linked_table'] === 'campus') {
    ...
    error_log("Error getting campus info: " . $e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        if ($action === 'approve_correction' || $action === 'reject_correction') {
            ...
            // Get correction details with campus verification
            $stmt = $pdo->prepare("
                SELECT 
                    ac.*,
            ...
            // Verify campus admin has access to this request
            if ($correction['campus_id'] != $campus_id) {
            ...
            if ($new_status === 'approved') {
                ...
                if ($existing) {
                    ...
                    $insert->execute([
                        ...
                        $correction['start_date'],
                        $corrected_status,
                        "Corrected from {$original_status} to {$corrected_status} by campus admin"
                    ]);
                }
            ...
            $update_correction = $pdo->prepare("
                ...
                SET status = ?, 
                    approved_by = ?, 
                    reason_details = CONCAT(COALESCE(reason_details, ''), ' | Campus Admin Notes: ', ?),
                    is_closed = 1,
                    updated_at = NOW()
                WHERE leave_id = ?
            ");
            ...
            if (!empty($correction['student_email'])) {
                ...
                $student_message .= "Campus Admin Notes: {$decision_notes}\n\n";
        ...
        if ($action === 'bulk_approve' || $action === 'bulk_reject') {
            ...
            // Verify all selected requests belong to this campus
            $placeholders = implode(',', array_fill(0, count($leave_ids), '?'));
            $verify_params = array_merge($leave_ids, [$campus_id]);
            ...
            $verify_sql = "
                ...
                LEFT JOIN students s ON ac.student_id = s.student_id
                LEFT JOIN classes c ON s.class_id = c.class_id
                WHERE ac.leave_id IN ({$placeholders}) 
                AND c.campus_id = ?
            ";
            ...
            if ($verify_result['count'] != count($leave_ids)) {
                throw new Exception("Some requests don't belong to your campus!");
            }
                    ...
                    if ($new_status === 'approved') {
                        ...
                        if ($existing) {
                            ...
                            $insert->execute([
                                ...
                                $correction['start_date'],
                                $corrected_status,
                                "Corrected from {$original_status} to {$corrected_status} by campus admin (bulk)"
                            ]);
                        }
                    }
        ...
        if ($action === 'approve_unlock' || $action === 'reject_unlock') {
            ...
            // Get unlock request details with campus verification using timetable
            $stmt = $pdo->prepare("
                SELECT al.*, 
                ...
                LEFT JOIN timetable tt ON al.teacher_id = tt.teacher_id 
                                       AND al.subject_id = tt.subject_id 
                                       AND al.lock_date BETWEEN tt.start_time AND tt.end_time
            ...
            if ($action === 'approve_unlock') {
                ...
                $update_attendance = $pdo->prepare("
                    UPDATE attendance 
                    SET locked = 0, remarks = CONCAT(COALESCE(remarks, ''), ' | Unlocked by campus admin')
            ...
            if (!empty($unlock['teacher_email'])) {
                ...
                $teacher_message .= "Decision: {$status_text}\n";
                $teacher_message .= "Campus Admin Notes: {$decision_notes}\n\n";
...
/* ================= FETCH PENDING CORRECTIONS (CAMPUS-SPECIFIC) ================= */
...
$pending_corrections->execute(['campus_id' => $campus_id]);
...
/* ================= FETCH PENDING UNLOCK REQUESTS (CAMPUS-SPECIFIC) ================= */
// Fixed: Using timetable table to get campus_id
$pending_unlocks = $pdo->prepare("
    ...
    AND EXISTS (
        ...
        AND tt.subject_id = sub.subject_id
        AND tt.campus_id = :campus_id
        AND tt.academic_term_id = :academic_term_id
    )
    ORDER BY 
        CASE 
...
/* ================= FETCH APPROVED/REJECTED HISTORY (CAMPUS-SPECIFIC) ================= */
$history = $pdo->prepare("
    (SELECT 
    ...
    WHERE (al.is_locked = 0 OR al.is_locked = 1)
    AND tt.campus_id = :campus_id
    GROUP BY al.lock_id)
...
$stats = $pdo->prepare("
    SELECT 
        (SELECT COUNT(*) 
         ...
         LEFT JOIN students s ON ac.student_id = s.student_id
         LEFT JOIN classes c ON s.class_id = c.class_id
         WHERE ac.status = 'pending' AND c.campus_id = ?) as pending_corrections,
        ...
        (SELECT COUNT(*) 
         ...
         LEFT JOIN students s ON ac.student_id = s.student_id
         LEFT JOIN classes c ON s.class_id = c.class_id
         WHERE ac.status = 'pending' AND ac.start_date < CURDATE() AND c.campus_id = ?) as past_corrections,
        
        (SELECT COUNT(*) 
         FROM attendance_lock al
         LEFT JOIN teachers t ON al.teacher_id = t.teacher_id
         LEFT JOIN subject sub ON al.subject_id = sub.subject_id
         WHERE al.is_locked = 1 AND al.lock_date < CURDATE()
         AND EXISTS (
             SELECT 1 FROM timetable tt
             WHERE tt.teacher_id = t.teacher_id
             AND tt.subject_id = sub.subject_id
...
");
$stats->execute([$campus_id, $campus_id, $academic_term_id, $campus_id, $campus_id, $academic_term_id]);
$stats_result = $stats->fetch(PDO::FETCH_ASSOC);

$total_pending = ($stats_result['pending_corrections'] ?? 0) + ($stats_result['pending_unlocks'] ?? 0);
$total_past = ($stats_result['past_corrections'] ?? 0) + ($stats_result['past_unlocks'] ?? 0);
...
<head>
    ...
    <title>Campus Admin - Attendance Management | <?php echo htmlspecialchars($campus_name); ?></title>
    <link rel="icon" type="image/png" href="../images.png">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
    /* ===========================================
       CSS VARIABLES (Same as super admin but with campus colors)
    =========================================== */
    :root {
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="page-header">
            ...
            <div class="campus-badge" style="background: #e3f2fd; color: var(--dark-color); padding: 8px 15px; border-radius: 20px;">
                <i class="fas fa-map-marker-alt"></i> Campus ID: <?php echo $campus_id; ?>
            </div>
        </div>

        <!-- STATS CARDS (Campus-Specific) -->
        <div class="stats-grid">
            <div class="stat-card correction">
                ...
                <div class="stat-info">
                    <h3><?php echo $stats_result['pending_corrections'] ?? 0; ?></h3>
            ...
            <div class="stat-card unlock">
                ...
                <div class="stat-info">
                    <h3><?php echo $stats_result['pending_unlocks'] ?? 0; ?></h3>
                    <p>Unlock Requests</p>
                </div>
            </div>
        ...
        <div id="tab1" class="tab-content active">
            <div class="table-wrapper">
                <div class="table-header">
                    <h2>
                        <i class="fas fa-clock"></i>
                        Pending Attendance Corrections - <?php echo htmlspecialchars($campus_name); ?>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>All attendance correction requests for <?php echo htmlspecialchars($campus_name); ?> have been processed.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div id="tab2" class="tab-content">
            <div class="table-wrapper">
                <div class="table-header">
                    <h2>
                        <i class="fas fa-unlock-alt"></i>
                        Teacher Unlock Requests - <?php echo htmlspecialchars($campus_name); ?>
                        <span style="font-size: 14px; color: #666; font-weight: normal; margin-left: 10px;">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>All teacher unlock requests for <?php echo htmlspecialchars($campus_name); ?> have been processed.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div id="tab3" class="tab-content">
            <div class="table-wrapper">
                <div class="table-header">
                    <h2>
                        <i class="fas fa-history"></i>
                        Recent Activity - <?php echo htmlspecialchars($campus_name); ?> (Last 50)
                    </h2>
                </div>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>No requests have been processed for <?php echo htmlspecialchars($campus_name); ?> yet.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <script>
        ...
        function viewUnlockDetails(data) {
            ...
            details += '<div class="detail-item"><div class="detail-label">Campus</div><div class="detail-value"><?php echo htmlspecialchars($campus_name); ?></div></div>';
