...
if ($current_faculty_id) {
    ...
    // Extract campus IDs and names for JavaScript
    foreach ($faculty_campuses as $campus) {
        $campus_ids[] = $campus['campus_id'];
        $campus_names[$campus['campus_id']] = $campus['campus_name'];
    }
}

// For backward compatibility, use first campus as primary
$primary_campus = $faculty_campuses[0] ?? null;
$campus_id = $primary_campus['campus_id'] ?? null;
$campus_name = $primary_campus['campus_name'] ?? '';
$campus_code = $primary_campus['campus_code'] ?? '';

$message = "";
$type = "";

/* =========================================================
   AJAX HANDLERS - Faculty Admin Version
========================================================= */

// AJAX: Get rooms by faculty and department (restricted to this faculty)
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_rooms') {
    header('Content-Type: application/json');
    
    try {
        $faculty_id = $current_faculty_id; // Force to current faculty
        $department_id = $_GET['department_id'] ?? null;
        
        $sql = "
            SELECT r.*, f.faculty_name, d.department_name, c.campus_name, c.campus_id
            FROM rooms r
            LEFT JOIN faculties f ON r.faculty_id = f.faculty_id
            LEFT JOIN departments d ON r.department_id = d.department_id
            LEFT JOIN campus c ON r.campus_id = c.campus_id
            WHERE r.status = 'available' AND r.faculty_id = ?
        ";
        ...
        $sql .= " ORDER BY c.campus_name, r.building_name, r.room_name";
...
// AJAX: Get departments by faculty (restricted to this faculty) - WITH CAMPUS NAME
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_departments') {
    ...
    try {
        ...
        $sql = "
            SELECT d.*, c.campus_name, c.campus_id
            FROM departments d
...
// AJAX handler for getting allocation details (with faculty check)
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_allocation_details' && isset($_GET['allocation_id'])) {
    ...
    try {
        ...
        $query = "
            ...
            LEFT JOIN campus rc ON ra.campus_id = rc.campus_id
            ...
            WHERE ra.allocation_id = ? AND r.faculty_id = ?
        ";
        
        $params = [$allocation_id, $current_faculty_id];
...
// AJAX handler for getting room allocations (restricted to this faculty)
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_room_allocations' && isset($_GET['room_id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            LEFT JOIN campus rc ON ra.campus_id = rc.campus_id
            LEFT JOIN rooms r ON ra.room_id = r.room_id
            WHERE ra.room_id = ? AND r.faculty_id = ?
            ORDER BY ra.start_date DESC
        ");
        $stmt->execute([$room_id, $current_faculty_id]);
...
/* =========================================================
   CRUD OPERATIONS FOR ALLOCATIONS - Faculty Admin Version
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_allocation') {
        try {
            ...
            // Verify room belongs to this faculty
            $stmt = $pdo->prepare("SELECT faculty_id, capacity, campus_id FROM rooms WHERE room_id = ?");
            $stmt->execute([$room_id]);
            $roomData = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if ($roomData['faculty_id'] != $current_faculty_id) {
            ...
            if ($department_id_val) {
                $deptCheck = $pdo->prepare("SELECT faculty_id, campus_id FROM departments WHERE department_id = ?");
                $deptCheck->execute([$department_id_val]);
                $deptData = $deptCheck->fetch(PDO::FETCH_ASSOC);
                
                if ($deptData['faculty_id'] != $current_faculty_id) {
            ...
            $check = $pdo->prepare("
                SELECT COUNT(*) FROM room_allocation ra
                JOIN rooms r ON ra.room_id = r.room_id
                WHERE ra.room_id = ? 
                AND r.faculty_id = ?
                AND (
                    (ra.start_date <= ? AND (ra.end_date >= ? OR ra.end_date IS NULL))
                    OR (ra.start_date <= ? AND (ra.end_date >= ? OR ra.end_date IS NULL))
                )
                AND ra.status = 'active'
            ");
            
            $check->execute([
                $room_id, $current_faculty_id,
            ...
            // Insert allocation - Use room's campus_id
            $stmt = $pdo->prepare("
            ...
            $stmt->execute([
                $room_id, $roomData['campus_id'], $academic_term_id, $department_id_val, $allocated_to,
    ...
    if ($action === 'update_allocation') {
        try {
            ...
            // Verify allocation belongs to this faculty
            $check = $pdo->prepare("
                SELECT ra.*, r.faculty_id, r.campus_id 
                FROM room_allocation ra
                JOIN rooms r ON ra.room_id = r.room_id
            ...
            if (!$alloc_data || $alloc_data['faculty_id'] != $current_faculty_id) {
            ...
            // Verify department belongs to this faculty
            if ($department_id_val) {
                $deptCheck = $pdo->prepare("SELECT faculty_id FROM departments WHERE department_id = ?");
                $deptCheck->execute([$department_id_val]);
                $deptFaculty = $deptCheck->fetchColumn();
                
                if ($deptFaculty != $current_faculty_id) {
                    throw new Exception("Selected department does not belong to your faculty");
                }
            }
            ...
            $check = $pdo->prepare("
                SELECT COUNT(*) FROM room_allocation ra
                JOIN rooms r ON ra.room_id = r.room_id
                WHERE ra.room_id = ? AND ra.allocation_id != ?
                AND r.faculty_id = ?
                AND (
                    (ra.start_date <= ? AND (ra.end_date >= ? OR ra.end_date IS NULL))
                    OR (ra.start_date <= ? AND (ra.end_date >= ? OR ra.end_date IS NULL))
            ...
            $check->execute([
                $room_id, $allocation_id, $current_faculty_id,
                $start_date, $start_date,
                $end_date ?? $start_date, $end_date ?? $start_date
            ]);
            ...
            // Update allocation - Use room's campus_id
            $stmt = $pdo->prepare("
    ...
    if ($action === 'delete_allocation') {
        try {
            ...
            // Verify allocation belongs to this faculty
            $check = $pdo->prepare("
                SELECT ra.room_id, r.faculty_id 
                FROM room_allocation ra
                JOIN rooms r ON ra.room_id = r.room_id
                WHERE ra.allocation_id = ?
            ");
            ...
            if (!$alloc || $alloc['faculty_id'] != $current_faculty_id) {
...
/* =========================================================
   FETCH DATA FOR DISPLAY - Faculty Admin Version
========================================================= */
...
// Fetch departments for this faculty (with campus name)
$departments = [];
$deptQuery = "
    SELECT d.department_id, d.department_name, d.department_code, c.campus_name, c.campus_id
    FROM departments d
    JOIN campus c ON d.campus_id = c.campus_id
    WHERE d.faculty_id = ? AND d.status = 'active'
    ORDER BY c.campus_name ASC, d.department_name ASC
";
$deptParams = [$current_faculty_id];
...
$roomQuery = "
    ...
    WHERE r.faculty_id = ?
";
$roomParams = [$current_faculty_id];

if (!empty($department_filter)) {
    $roomQuery .= " AND r.department_id = ?";
    $roomParams[] = $department_filter;
}

$roomQuery .= " ORDER BY c.campus_name, d.department_name, r.building_name, r.room_name";
...
// Build main query for allocations (restricted to this faculty)
$query = "
    SELECT 
        ra.*,
        r.room_name,
        ...
        r.campus_id,
        r.faculty_id,
        r.department_id as room_department_id,
        rc.campus_name,
        rc.campus_code,
        f.faculty_name,
        f.faculty_code,
...
// Stats for this faculty
$statsQuery = "SELECT COUNT(*) FROM room_allocation ra JOIN rooms r ON ra.room_id = r.room_id WHERE r.faculty_id = ?";
$statsParams = [$current_faculty_id];
...
<head>
<meta charset="UTF-8">
<title>Room Allocation Management - <?= htmlspecialchars($faculty_name) ?> | Hormuud University</title>
...
@media (max-width: 768px) {
  ...
  .faculty-badge, .campus-badge, .multi-campus-badge {
    margin-left: 0;
    margin-top: 5px;
  }
...
<div class="main-content">
  <div class="page-header">
    <h1>
      ...
      <?php if (count($faculty_campuses) == 1): ?>
        <span class="campus-badge"><i class="fas fa-map-marker-alt"></i> <?= htmlspecialchars($campus_name) ?></span>
      <?php elseif (count($faculty_campuses) > 1): ?>
        <span class="multi-campus-badge"><i class="fas fa-map-marker-alt"></i> <?= count($faculty_campuses) ?> Campuses</span>
      <?php endif; ?>
    </h1>
  ...
  <div class="stats-container">
    ...
    <div class="stat-card active">
      <div class="stat-icon active"><i class="fas fa-play-circle"></i></div>
      <div class="stat-info">
        <h3>Current</h3>
  ...
  <div class="filters-container">
    <div class="filter-header">
      <h3><i class="fas fa-filter"></i> Filter Allocations - <?= htmlspecialchars($faculty_name) ?></h3>
    </div>
    ...
    <form method="GET" class="filter-form" id="filterForm">
          ...
          <select id="department" name="department" class="filter-input">
            ...
            <?php foreach($departments as $dept): ?>
              <option value="<?= $dept['department_id'] ?>" 
                <?= $department_filter == $dept['department_id'] ? 'selected' : '' ?>>
      ...
      <div class="filter-group">
        ...
        <div style="position:relative;">
          <i class="fas fa-door-closed filter-icon"></i>
          <select id="room" name="room" class="filter-input">
            <option value="">All Rooms</option>
            <?php foreach($allRooms as $room): ?>
              <option value="<?= $room['room_id'] ?>" 
                <?= $room_filter == $room['room_id'] ? 'selected' : '' ?>>
                <?= htmlspecialchars($room['building_name'] . ' - ' . $room['room_name'] . ' (' . $room['room_code'] . ')') ?>
                <?php if (isset($room['campus_name'])): ?> - <?= htmlspecialchars($room['campus_name']) ?><?php endif; ?>
              </option>
            <?php endforeach; ?>
          </select>
        </div>
  ...
  <div class="table-wrapper">
    <div class="table-header">
      <h3><i class="fas fa-list"></i> Allocations List - <?= htmlspecialchars($faculty_name) ?></h3>
          ...
          <?php if($allocations): ?>
            ...
            <tr>
              ...
              <td>
                ...
                <div class="room-code"><?= htmlspecialchars($a['room_code'] ?? '') ?>
                  <?php if (isset($a['campus_name'])): ?> | <?= htmlspecialchars($a['campus_name']) ?><?php endif; ?>
                </div>
              </td>
              <td>
                <?= htmlspecialchars($a['department_name'] ?? '<span style="color: #999; font-style: italic;">All University</span>') ?>
                <?php if (isset($a['campus_name']) && !$a['department_name']): ?> (<?= htmlspecialchars($a['campus_name']) ?>)<?php endif; ?>
              </td>
              <td>
            ...
            <tr>
              <td colspan="10">
                <div class="empty-state">
                  ...
                  <h3>No allocations found for <?= htmlspecialchars($faculty_name) ?></h3>
                  <p>
                    <?php if(!empty($search) || !empty($department_filter) || !empty($room_filter) || !empty($term_filter) || !empty($status_filter) || !empty($date_from) || !empty($date_to)): ?>
...
<div class="modal" id="addAllocationModal">
  <div class="modal-content">
    ...
    <h2><i class="fas fa-plus-circle"></i> Create Room Allocation - <?= htmlspecialchars($faculty_name) ?></h2>
    ...
    <form method="POST" id="addAllocationForm" onsubmit="return validateAllocationForm('add')">
      ...
      <div class="form-grid">
        <div class="selection-group">
          ...
          <input type="text" class="form-control" value="<?= htmlspecialchars($faculty_name) ?> (<?= htmlspecialchars($faculty_code) ?>)" disabled readonly>
          <?php if (count($faculty_campuses) > 1): ?>
            <small style="display: block; margin-top: 5px; color: var(--hormuud-gold);">
              <i class="fas fa-info-circle"></i> This faculty has <?= count($faculty_campuses) ?> campuses. Select department to see rooms.
            </small>
          <?php endif; ?>
        </div>
        ...
        <div class="selection-group">
          ...
          <select id="add_department" name="department_id" class="form-control" required onchange="loadRooms('add')">
...
<div class="modal" id="editAllocationModal">
  <div class="modal-content">
    ...
    <h2><i class="fas fa-edit"></i> Edit Allocation - <?= htmlspecialchars($faculty_name) ?></h2>
    ...
    <form method="POST" id="editAllocationForm" onsubmit="return validateAllocationForm('edit')">
      ...
      <div class="form-grid">
        ...
        <div class="selection-group">
          <label><i class="fas fa-building"></i> Department *</label>
          <select id="edit_department" name="department_id" class="form-control" required onchange="loadRooms('edit')">
            <option value="">-- Select Department --</option>
            <?php foreach($departments as $dept): ?>
              <option value="<?= $dept['department_id'] ?>">
                <?= htmlspecialchars($dept['department_name']) ?> (<?= htmlspecialchars($dept['campus_name']) ?>)
...
let campusNames = <?= json_encode($campus_names) ?>;

// Document ready
$(document).ready(function() {
    $('#department, #room, #term, #status').select2({
    ...
    $('#department, #room, #term, #status').on('change', function() {
...
function validateStudentCount(prefix) {
    ...
    if (capacity) {
        ...
        if (studentCount > parseInt(capacity)) {
...
function loadRooms(prefix) {
    ...
    fetch(`?ajax=get_rooms&department_id=${departmentId}`)
            ...
            if (data.status === 'success') {
                ...
                if (data.rooms.length === 0) {
                    ...
                    data.rooms.forEach(room => {
                        const option = document.createElement('option');
                        option.value = room.room_id;
                        option.textContent = `${room.building_name} - ${room.room_name} (${room.room_code}) - Capacity: ${room.capacity} ${room.campus_name ? ' - ' + room.campus_name : ''}`;
...
function openEditAllocationModal(id, roomId, deptId, termId, allocatedTo, startDate, endDate, remarks, studentCount, status, roomCapacity) {
                        ...
                        <div class="detail-item">
                            ...
                            <div class="detail-value">
                                ...
                                <div style="font-size: 12px; color: #666; margin-top: 5px;">
                                    ...
                                    ${a.campus_name ? ' | Campus: ' + a.campus_name : ''}
                                </div>
                            </div>
                        </div>
