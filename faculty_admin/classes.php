...
if (isset($_GET['ajax_action']) && isset($_SESSION['csrf_token'])) {
    ...
    // Get departments by faculty and campus
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        // Validate department belongs to faculty and campus
        $check = $pdo->prepare("SELECT COUNT(*) FROM departments WHERE department_id = ? AND faculty_id = ? AND campus_id = ?");
        $check->execute([$department_id, $faculty_id, $campus_id]);
        ...
        if ($action === 'add') {
            ...
            $check = $pdo->prepare("
                ...
                WHERE class_name = ? AND campus_id = ? AND program_id = ? AND study_mode = ?
            ");
        ...
        if ($action === 'update') {
            ...
            $check = $pdo->prepare("SELECT COUNT(*) FROM classes WHERE class_id = ? AND faculty_id = ?");
            $check->execute([$class_id, $faculty_id]);
            if ($check->fetchColumn() == 0) {
            ...
            $check = $pdo->prepare("
                SELECT COUNT(*) FROM classes 
                WHERE class_name = ? AND campus_id = ? AND program_id = ? AND study_mode = ? AND class_id != ?
            ");
            ...
            if ($check->fetchColumn() > 0) {
                ...
                $stmt = $pdo->prepare("
                    UPDATE classes 
                    SET class_name=?, campus_id=?, department_id=?, program_id=?, study_mode=?, status=?, updated_at=NOW()
                    WHERE class_id=?
                ");
                $stmt->execute([
                    $class_name,
...
<head>
<meta charset="UTF-8">
<title>Class Management | Faculty Admin - <?= htmlspecialchars($faculty_name) ?> | Hormuud University</title>
...
.edit-btn:hover {
  background: linear-gradient(135deg, #005fa3, #1976d2);
  transform: translateY(-2px);
}
  ...
  <div class="filters-container">
    ...
    <div class="filter-form">
      <div class="filter-group">
        ...
        <select class="filter-select" id="filter_campus">
      ...
      <div class="filter-group">
        ...
        <select class="filter-select" id="filter_department" disabled>
      ...
      <table>
        ...
        <tbody id="classesTableBody">
          ...
          <tr>
            ...
            <td>
              <?= $c['campus_name'] ?>
              <span class="campus-badge"><i class="fas fa-map-marker-alt"></i> Campus</span>
            </td>
            <td><?= $c['study_mode'] ?></td>
            <td>
  ...
  <div class="modal-content">
    ...
    <form class="modal-form" method="POST" id="addForm" onsubmit="return validateForm('add')">
      ...
      <div class="form-group">
        ...
        <select id="add_campus" name="campus_id" required onchange="loadDepartments(this.value, 'add')">
      ...
      <div class="form-group">
        ...
        <select id="add_department" name="department_id" required onchange="loadPrograms(this.value, 'add')" disabled>
          <option value="">Select Campus First</option>
        </select>
      </div>
  ...
  <div class="modal-content">
    ...
    <form class="modal-form" method="POST" id="editForm" onsubmit="return validateForm('edit')">
      ...
      <div class="form-group">
        ...
        <select id="edit_campus" name="campus_id" required onchange="loadDepartments(this.value, 'edit')">
          <option value="">Select Campus</option>
          <?php foreach($faculty_campuses as $c): ?>
            <option value="<?= $c['campus_id'] ?>"><?= htmlspecialchars($c['campus_name']) ?></option>
          <?php endforeach; ?>
      ...
      <div class="form-group">
        ...
        <select id="edit_department" name="department_id" required onchange="loadPrograms(this.value, 'edit')" disabled>
          <option value="">Select Campus First</option>
        </select>
      </div>
...
function openViewModal(id, name, program, dept, campus, mode, status, created, updated) {
...
function openEditModal(id, name, campusId, deptId, programId, mode, status) {
    ...
    // Load dependencies
    loadDepartments(campusId, 'edit', deptId, programId);
    openModal('editModal');
}
...
// Filter functions
function loadFilterDepartments(campusId) {
    ...
    fetch(`${currentFile}?ajax_action=get_departments&campus_id=${campusId}&csrf_token=${csrfToken}`)
...
document.getElementById('filter_campus').addEventListener('change', function(e) {
    loadFilterDepartments(e.target.value);
});
...
function resetFilters() {
    ...
    document.getElementById('filter_department').innerHTML = '<option value="">Select Campus First</option>';
    document.getElementById('filter_department').disabled = true;
    document.getElementById('filter_program').innerHTML = '<option value="">Select Department First</option>';
...
function loadDepartments(campusId, prefix, selectedDept = null, selectedProgram = null) {
            ...
            if (data.status === 'success') {
                ...
                if (selectedDept) {
                    setTimeout(() => {
                        ...
                        loadPrograms(selectedDept, prefix, selectedProgram);
                    }, 100);
                }
            } else {
                showAlert('Error loading departments', 'error');
...
function loadPrograms(deptId, prefix, selectedProgram = null) {
    const progSelect = document.getElementById(prefix + '_program');
    ...
    fetch(`${currentFile}?ajax_action=get_programs&department_id=${deptId}&campus_id=${campusId}&csrf_token=${csrfToken}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
