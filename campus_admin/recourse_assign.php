...
$user_campus_id = null;
if ($user_role === 'campus_admin' && isset($_SESSION['user']['linked_id']) && $_SESSION['user']['linked_table'] === 'campus') {
...
if (isset($_GET['ajax'])) {
    ...
    // Security check for campus_admin in AJAX requests
    if ($user_role === 'campus_admin' && $user_campus_id) {
        // Check if campus_id is provided and matches user's campus
    ...
    if ($_GET['ajax'] == 'get_faculties_by_campus') {
        ...
        // For campus_admin, force their campus ID
        if ($user_role === 'campus_admin' && $user_campus_id) {
    ...
    if ($_GET['ajax'] == 'get_recourse_hierarchy') {
        ...
        // Security check for campus_admin
        if ($user_role === 'campus_admin' && $user_campus_id && $recourse) {
            if ($recourse['recourse_campus_id'] != $user_campus_id) {
    ...
    if ($_GET['ajax'] == 'get_statistics') {
        if ($user_role === 'super_admin') {
            ...
            $total = $active = $completed = $cancelled = 0;
        }
...
// For campus_admin, automatically filter by their campus
if ($user_role === 'campus_admin' && $user_campus_id) {
    if (empty($filter_campus)) {
        $filter_campus = $user_campus_id;
    } else if ($filter_campus != $user_campus_id) {
        $message = "❌ You are only authorized to view data from your assigned campus.";
        $type = "error";
        $filter_campus = $user_campus_id;
    }
}
...
// Add campus admin restriction to WHERE conditions
if ($user_role === 'campus_admin' && $user_campus_id) {
    ...
    try {
        ...
        if ($user_role === 'campus_admin' && $user_campus_id) {
            if ($_POST['action'] === 'add' || $_POST['action'] === 'update') {
                // Check if recourse campus is their campus
                if ($_POST['recourse_campus_id'] != $user_campus_id) {
                    throw new Exception("❌ You are only authorized to add recourse for your campus!");
                }
                
                // Also check student's campus
                $student_check = $pdo->prepare("SELECT campus_id FROM students WHERE student_id = ?");
                $student_check->execute([$_POST['student_id']]);
                $student_campus = $student_check->fetchColumn();
                
                if ($student_campus != $user_campus_id) {
                    throw new Exception("❌ This student is not from your campus!");
                }
            } else if ($_POST['action'] === 'delete') {
                // Check if the recourse record belongs to their campus
                $recourse_check = $pdo->prepare("SELECT recourse_campus_id FROM recourse_student WHERE recourse_id = ?");
                $recourse_check->execute([$_POST['recourse_id']]);
                $recourse_campus = $recourse_check->fetchColumn();
                
                if ($recourse_campus != $user_campus_id) {
...
// Get user's campus name for display
$user_campus_name = '';
if ($user_role === 'campus_admin' && $user_campus_id) {
...
if ($user_role === 'super_admin') {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM recourse_student WHERE recourse_campus_id = ?");
    $stmt->execute([$user_campus_id]);
    $total_recourses = $stmt->fetchColumn();
    
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM recourse_student WHERE recourse_campus_id = ? AND status = 'active'");
    $stmt->execute([$user_campus_id]);
    $active_recourses = $stmt->fetchColumn();
    
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM recourse_student WHERE recourse_campus_id = ? AND status = 'completed'");
    $stmt->execute([$user_campus_id]);
    $completed_recourses = $stmt->fetchColumn();
    
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM recourse_student WHERE recourse_campus_id = ? AND status = 'cancelled'");
    $stmt->execute([$user_campus_id]);
    $cancelled_recourses = $stmt->fetchColumn();
} else {
    $total_recourses = $active_recourses = $completed_recourses = $cancelled_recourses = 0;
}
...
<body>
    <div class="main-content">
        ...
        <div class="top-bar no-print">
            <div class="top-left">
                ...
                <div class="user-info">
                    <span><i class="fas fa-user"></i> <?= htmlspecialchars($_SESSION['user']['username'] ?? 'User') ?> 
                    (<?= htmlspecialchars(ucfirst(str_replace('_', ' ', $user_role))) ?>)</span>
                    <?php if ($user_role === 'campus_admin' && $user_campus_name): ?>
                        <span style="color:#0072CE;">|</span>
                        <span><i class="fas fa-university"></i> <?= htmlspecialchars($user_campus_name) ?></span>
                    <?php endif; ?>
                </div>
            </div>
            <div>
            ...
            <form method="GET" id="filterForm">
                <div class="grid">
                    ...
                    <div>
                        ...
                        <select name="filter_campus" id="filter_campus" onchange="onFilterCampusChange(this.value)" <?= ($user_role === 'campus_admin' && $user_campus_id) ? 'disabled' : '' ?>>
                            ...
                            if ($user_role === 'super_admin') {
                                ...
                                foreach($campuses as $c): 
                                    if ($c['campus_id'] == $user_campus_id): ?>
                                        <option value="<?= $c['campus_id'] ?>" selected>
                                            <?= htmlspecialchars($c['campus_name']) ?>
                                        </option>
                                    <?php endif;
                                endforeach;
                    ...
                    <div class="grid">
                        ...
                        <div class="hierarchy-section">
                            ...
                            <div class="hierarchy-grid">
                                ...
                                <div>
                                    <label><i class="fas fa-university"></i> Campus*</label>
                                    <select id="modal_campus" required onchange="onModalCampusChange(this.value)" <?= ($user_role === 'campus_admin' && $user_campus_id) ? 'disabled' : '' ?>>
                        ...
                        <div>
                            ...
                            <select name="recourse_campus_id" id="recourse_campus_id" required onchange="onRecourseCampusChange()" <?= ($user_role === 'campus_admin' && $user_campus_id) ? 'disabled' : '' ?>>
                                <option value="">Select Campus</option>
                                <?php 
                                if ($user_role === 'super_admin') {
                                    foreach($campuses as $c): ?>
    ...
    function onFilterCampusChange(campusId) {
        if (userRole === 'campus_admin' && userCampusId) {
            return; // Campus admin can't change campus
        }
        ...
        if (addModal) {
            ...
            openModal = function(id) {
                if (id === 'addModal') {
                    ...
                    // Auto-select campus for campus admin
                    if (userRole === 'campus_admin' && userCampusId) {
                        document.getElementById('modal_campus').value = userCampusId;
                        onModalCampusChange(userCampusId);
                        
                        document.getElementById('recourse_campus_id').value = userCampusId;
                        onRecourseCampusChange();
                    }
                }
