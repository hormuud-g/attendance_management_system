...
// ✅ Access Control - Modified for Department Admin
...
if ($role === 'department_admin') {
    // Assuming department_admin's linked_id points to department_id
    $user_department_id = $user['linked_id'] ?? null;
    
    // If not set in session, fetch from database
    if (!$user_department_id) {
        $stmt = $pdo->prepare("SELECT department_id FROM department_admins WHERE user_id = ?");
        $stmt->execute([$user['user_id'] ?? 0]);
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] == 'get_faculties') {
        ...
        // For department admin, filter by their department's faculty
    ...
    if ($_GET['ajax'] == 'get_departments') {
        ...
        // For department admin, only return their department
        if ($role === 'department_admin' && $user_department_id) {
            $stmt = $pdo->prepare("
    ...
    if ($_GET['ajax'] == 'get_programs') {
        ...
        // For department admin, only show programs in their department
    ...
    if ($_GET['ajax'] == 'get_classes') {
        ...
        // For department admin, only show classes in their department
        if ($role === 'department_admin' && $user_department_id && $department_id != $user_department_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
            exit;
        }
    ...
    if ($_GET['ajax'] == 'get_subjects') {
        ...
        // For department admin, only show subjects in their department
        if ($role === 'department_admin' && $user_department_id && $department_id != $user_department_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized access']);
            exit;
        }
    ...
    // GET TEACHERS - CORRECTED for department admin
    if ($_GET['ajax'] == 'get_teachers') {
        // Since there's no direct department_id in teachers table,
        // for department admin, we'll show all teachers
        // You can modify this if you have a different relationship
        $stmt = $pdo->prepare("
    ...
    // GET ROOMS BY CAMPUS - CORRECTED based on your table structure
    if ($_GET['ajax'] == 'get_rooms') {
        ...
        // For department admin, show rooms in their department or campus-wide
        if ($role === 'department_admin' && $user_department_id) {
            // Show rooms that either belong to this department OR have no department (campus-wide)
            ...
            $stmt->execute([$campus_id, $user_department_id]);
        } else {
            // Start building query for super_admin/campus_admin
            $sql = "SELECT room_id, room_name, room_code, room_type, capacity,
                           building_name, floor_no
                    FROM rooms 
                    WHERE campus_id = ? 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (in_array($action, ['add', 'update'])) {
            ...
            // For department admin, verify they can only modify their department's timetable
            if ($role === 'department_admin' && $department_id != $user_department_id) {
                throw new Exception("❌ You can only manage timetable entries for your department.");
            }
        ...
        elseif ($action === 'delete') {
            ...
            // For department admin, verify they can only delete their department's timetable entries
            if ($role === 'department_admin') {
                $stmt = $pdo->prepare("SELECT department_id FROM timetable WHERE timetable_id = ?");
                $stmt->execute([$id]);
                $timetable_dept_id = $stmt->fetchColumn();
                
                if ($timetable_dept_id != $user_department_id) {
                    throw new Exception("❌ You can only delete timetable entries for your department.");
                }
            }
...
$selectedDepartment = $_GET['department_id'] ?? ($role === 'department_admin' ? $user_department_id : null);
...
/* ===========================================
   FETCH STATIC DATA
=========================================== */
// Campuses (ACTIVE ONLY) - Modified for department admin
if ($role === 'super_admin') {
    ...
    $campuses = $pdo->prepare("
        ...
        FROM campus c
        JOIN departments d ON c.campus_id = d.campus_id
        WHERE d.department_id = ? AND c.status = 'active'
    ");
    $campuses->execute([$user_department_id]);
    $campuses = $campuses->fetchAll(PDO::FETCH_ASSOC);
}
...
if ($selectedCampus) {
    if ($role === 'department_admin' && $user_department_id) {
        ...
        ");
        $filterFaculties->execute([$user_department_id]);
    } else {
        $filterFaculties = $pdo->prepare("
            SELECT DISTINCT f.faculty_id, f.faculty_name
            FROM faculties f
...
if ($selectedFaculty && $selectedCampus) {
    if ($role === 'department_admin' && $user_department_id) {
        ...
        ");
        $filterDepartments->execute([$user_department_id, $selectedFaculty, $selectedCampus]);
    } else {
        $filterDepartments = $pdo->prepare("
            SELECT department_id, department_name
            FROM departments
...
if ($selectedDepartment && (!$role === 'department_admin' || $selectedDepartment != $user_department_id)) {
    $whereConditions[] = "t.department_id = ?";
...
$timetablesQuery = "
    SELECT t.*, 
           ...
           r.room_name, r.room_code, r.room_type, r.building_name, r.floor_no,
           tr.teacher_name,
           at.term_name
    FROM timetable t
...
if ($role === 'department_admin' && $user_department_id) {
    $stmt = $pdo->prepare("
        SELECT d.department_name, c.campus_name, f.faculty_name
        FROM departments d
...
<head>
<meta charset="UTF-8">
<title>Department Timetable | Hormuud University</title>
...
<div class="main-content">
    <div class="page-header">
        <div>
            ...
            <?php if($role === 'department_admin' && $departmentInfo): ?>
                <div class="department-badge">
                    <i class="fas fa-building"></i>
                    <?= htmlspecialchars($departmentInfo['department_name']) ?> - 
                    <?= htmlspecialchars($departmentInfo['faculty_name']) ?> - 
                    <?= htmlspecialchars($departmentInfo['campus_name']) ?>
                </div>
            <?php endif; ?>
        </div>
            ...
            <div class="grid">
                ...
                <div>
                    ...
                    <select name="campus_id" id="filter_campus" onchange="onFilterCampusChange()" 
                            <?= ($role === 'department_admin' && count($campuses) === 1) ? 'disabled' : '' ?>>
                ...
                <div>
                    ...
                    <select name="faculty_id" id="filter_faculty" 
                            ...
                            <?= empty($selectedCampus) && $role !== 'department_admin' ? 'disabled' : '' ?>>
                ...
                <!-- DEPARTMENT - For department admin, this is hidden/disabled -->
                <div <?= $role === 'department_admin' ? 'style="display:none;"' : '' ?>>
                                        ...
                                        <?php if(!empty($schedule['room_name'])): ?>
                                            ...
                                            <?php if(!empty($schedule['building_name'])): ?>
                                                <div style="font-size: 11px; color: #666;">
                                                    ...
                                                    <?php if(!empty($schedule['floor_no'])): ?>
                                                        , Floor <?= htmlspecialchars($schedule['floor_no']) ?>
                                                    <?php endif; ?>
                                                </div>
                                            <?php endif; ?>
                                        <?php else: ?>
        ...
        <form method="POST" id="timetableForm" onsubmit="return validateForm()">
            ...
            <div class="form-grid">
                ...
                <div>
                    ...
                    <?php if($role === 'department_admin' && count($campuses) === 1): ?>
                        <input type="hidden" name="campus_id" value="<?= $campuses[0]['campus_id'] ?>">
                    <?php endif; ?>
                </div>
                ...
                <div>
                    ...
                    <?php if($role === 'department_admin' && $user_department_id): ?>
                        <input type="hidden" name="department_id" id="department_id_hidden" value="<?= $user_department_id ?>">
                    <?php endif; ?>
                </div>
...
const userRole = '<?= $role ?>';
const userDepartmentId = '<?= $user_department_id ?>';
...
function openModal() {
    ...
    // For department admin, auto-select their department
    if (userRole === 'department_admin' && userDepartmentId) {
        // Set campus if only one option
        const campusSelect = document.getElementById('campus_id');
        if (campusSelect.options.length === 2) { // First option is "Select Campus"
            campusSelect.value = campusSelect.options[1].value;
            onCampusChange();
        }
    }
        ...
        .then(data => {
            ...
            if (data.status === 'success' && data.departments.length > 0) {
                data.departments.forEach(dept => {
                    ...
                    // For department admin, only show their department
                    if (userRole === 'department_admin' && dept.department_id != userDepartmentId) {
                ...
                // For department admin, auto-select their department
                if (userRole === 'department_admin' && userDepartmentId) {
                    deptSelect.value = userDepartmentId;
                    onDepartmentChange();
                }
            } else {
            ...
            if (data.status === 'success' && data.rooms.length > 0) {
                data.rooms.forEach(room => {
                    ...
                    if (room.building_name) {
                        displayText += ` - ${room.building_name}`;
                    }
                    if (room.floor_no) {
                        displayText += ` Floor ${room.floor_no}`;
                    }
                    if (room.capacity) {
...
function resetHierarchy() {
    ...
    // Reset teacher and room selects
    teacherSelect.innerHTML = '<option value="">Select Teacher</option>';
    teacherSelect.innerHTML += '<option value="">No Teacher</option>';
    teacherSelect.disabled = false;
...
function validateForm() {
    ...
    if (!campusId && document.getElementById('campus_id').type !== 'hidden') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // For department admin, set initial values
    if (userRole === 'department_admin' && userDepartmentId) {
        // Trigger campus change if campus is selected
