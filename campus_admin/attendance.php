<?php
/**
 * ATTENDANCE MANAGEMENT SYSTEM - CAMPUS ADMIN VERSION
 * 
 * FIXED: 
 * - Departments load immediately when campus is selected
 * - Class names show study mode (Part-time/Full-time)
 * - No need to click "Load Students" to see departments
 */
...
if (!isset($_SESSION['user']) || !in_array(strtolower($_SESSION['user']['role']), ['super_admin', 'campus_admin'])) {
    session_destroy();
    header("Location: ../login.php?error=unauthorized");
    exit;
}

// If user is campus_admin, get their campus_id from users table
$campus_admin_campus_id = null;
if (isset($_SESSION['user']) && strtolower($_SESSION['user']['role']) === 'campus_admin') {
    ...
    if ($user_data) {
        $campus_admin_campus_id = $user_data['linked_id'];
    } else {
        session_destroy();
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] == 'get_faculties_by_campus') {
        ...
        // If campus admin, restrict to their campus
        if ($campus_admin_campus_id && $campus_id != $campus_admin_campus_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized campus']);
            exit;
        }
    ...
    if ($_GET['ajax'] == 'get_departments_by_faculty') {
        ...
        // If campus admin, verify campus access
        if ($campus_admin_campus_id && $campus_id != $campus_admin_campus_id) {
            echo json_encode(['status' => 'error', 'message' => 'Unauthorized campus']);
            exit;
        }
    ...
    if ($_GET['ajax'] == 'search_students') {
        ...
        // If campus admin, only show students from their campus
        if ($campus_admin_campus_id) {
            $stmt = $pdo->prepare("
            ...
            ");
            $searchTerm = "%$query%";
            $stmt->execute([$searchTerm, $searchTerm, $campus_admin_campus_id]);
        } else {
            $stmt = $pdo->prepare("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        // If campus admin, verify they have access to this campus
        if ($campus_admin_campus_id && $campus_id != $campus_admin_campus_id) {
        ...
        if ($action === 'request_correction') {
            ...
            // If campus admin, verify student is in their campus
            if ($campus_admin_campus_id) {
                ...
                if (!$studentCampus || $studentCampus['campus_id'] != $campus_admin_campus_id) {
                    throw new Exception("Student is not in your authorized campus!");
                }
            }
...
// ✅ Teachers - If campus admin, filter teachers by campus through timetable
if ($campus_admin_campus_id) {
    $stmt = $pdo->prepare("
    ...
    ");
    $stmt->execute([$campus_admin_campus_id]);
...
// ✅ Campuses - If campus admin, only show their campus
if ($campus_admin_campus_id) {
    $stmt = $pdo->prepare("
...
// ✅ Cascade filters - INITIALLY EMPTY, WILL BE POPULATED BY AJAX
...
// ✅ Get Email Logs - If campus admin, only show logs for their campus students
try {
    // Create email_logs table if not exists
    $tableCheck = $pdo->query("SHOW TABLES LIKE 'email_logs'")->fetch();
    if (!$tableCheck) {
...
// ✅ Attendance Views (INCLUDING RECOURSE STUDENTS) - If campus admin, filter by campus
if ($campus_admin_campus_id) {
    $stmt = $pdo->prepare("
        SELECT 
            a.attendance_date, 
...
// Get all recourse students - If campus admin, filter by their campus
if ($campus_admin_campus_id) {
    $stmt = $pdo->prepare("
        SELECT 
            rs.recourse_id,
            ...
            oc2.study_mode as original_study_mode,
            ...
            rc2.study_mode as recourse_study_mode,
            rs.academic_term_id,
            at.term_name,
            rs.status,
            rs.created_at
...
// Teacher View - If campus admin, filter teachers by campus through timetable relationship
$teacher_view = [];
...
// If campus admin, only show corrections for their campus students
if ($campus_admin_campus_id) {
    $stmt = $pdo->prepare("
        SELECT 
...
if (isset($_GET['send_absence_emails']) && $_GET['send_absence_emails'] == 'now') {
    ...
    if ($campus_admin_campus_id) {
        ...
        ");
        $stmt->execute([$today, $academic_term_id, $campus_admin_campus_id]);
    } else {
        $stmt = $pdo->prepare("
            SELECT a.student_id, a.subject_id, 
                   (SELECT COUNT(*) 
...
<head>
    ...
    <title>Attendance Management | Campus Admin</title>
    ...
    <style>
        ...
        .correction-modal-content::-webkit-scrollbar-track {
            background: #f1f1f1;
            border-radius: 4px;
        }
        .table-wrapper::-webkit-scrollbar-thumb,
        .correction-modal-content::-webkit-scrollbar-thumb {
            background: var(--blue);
            border-radius: 4px;
        }
        .table-wrapper::-webkit-scrollbar-thumb:hover,
        .correction-modal-content::-webkit-scrollbar-thumb:hover {
            background: var(--green);
        }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <h1>
                ...
                <?php if ($campus_admin_campus_id): ?>
                    <span style="font-size: 16px; color: var(--green);">
                        <i class="fas fa-map-marker-alt"></i> Campus Admin
                    </span>
                <?php endif; ?>
            </h1>
                        ...
                        <?php if (!empty($attendance_view)): ?>
                            <?php foreach($attendance_view as $a): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="study-mode-badge <?php echo strtolower(str_replace('-', '', $a['study_mode'] ?? 'full-time')); ?>">
                                            <?php echo htmlspecialchars($a['study_mode'] ?? 'Full-time'); ?>
                                        </span>
                                    </td>
                        ...
                        <?php if (!empty($recourse_students)): ?>
                            <?php $i = 1; foreach($recourse_students as $rs): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($rs['original_class']); ?></td>
                                    <td>
                                        <span class="study-mode-badge <?php echo strtolower(str_replace('-', '', $rs['original_study_mode'] ?? 'full-time')); ?>">
                                            <?php echo htmlspecialchars($rs['original_study_mode'] ?? 'Full-time'); ?>
                                        </span>
                                    </td>
                                    <td><?php echo htmlspecialchars($rs['recourse_class']); ?></td>
                                    <td>
                                        <span class="study-mode-badge <?php echo strtolower(str_replace('-', '', $rs['recourse_study_mode'] ?? 'full-time')); ?>">
                                            <?php echo htmlspecialchars($rs['recourse_study_mode'] ?? 'Full-time'); ?>
                                        </span>
                                    </td>
                                    <td>
    ...
    $(document).ready(function() {
        ...
        // ===========================================
        // ATTENDANCE HIERARCHY FUNCTIONS - LOADS IMMEDIATELY
        // ===========================================
        ...
        $('#campus_id').change(function() {
            ...
            if(campusId) {
                // Load faculties for this campus
                $.ajax({
                    ...
                    success: function(data) {
                        if(data.status === 'success') {
                            $('#faculty_id').html('<option value="">Select Faculty</option>').prop('disabled', false);
                            $.each(data.faculties, function(index, faculty) {
                                $('#faculty_id').append('<option value="'+faculty.faculty_id+'">'+faculty.faculty_name+'</option>');
                            });
                            
                            // If there's only one faculty, auto-select it
                            if (data.faculties.length === 1) {
                                $('#faculty_id').val(data.faculties[0].faculty_id).trigger('change');
                            }
                            
                            // If there's a selected value from POST
                    ...
                    success: function(data) {
                        if(data.status === 'success') {
                            ...
                            if (data.departments.length === 1) {
                                $('#department_id').val(data.departments[0].department_id).trigger('change');
                            }
        ...
        $('#department_id').change(function() {
            ...
            if(departmentId && facultyId && campusId) {
                $.ajax({
                    ...
                    success: function(data) {
                        if(data.status === 'success') {
                            $('#program_id').html('<option value="">Select Program</option>').prop('disabled', false);
                            $.each(data.programs, function(index, program) {
                                $('#program_id').append('<option value="'+program.program_id+'">'+program.program_name+'</option>');
                            });
                            
                            // If there's only one program, auto-select it
                            if (data.programs.length === 1) {
                                $('#program_id').val(data.programs[0].program_id).trigger('change');
                            }
        ...
        // Program Change - Load Classes with Study Mode
        $('#program_id').change(function() {
            ...
            if(programId && departmentId && facultyId && campusId) {
                $.ajax({
                    ...
                    success: function(data) {
                        if(data.status === 'success') {
                            $('#class_id').html('<option value="">Select Class</option>').prop('disabled', false);
                            $.each(data.classes, function(index, cls) {
                                $('#class_id').append('<option value="'+cls.class_id+'">' + cls.display_name + '</option>');
                            });
